...
### 브랜드 대행사 설정
RCS 브랜드 승인 완료 후, 대행사를 ‘엔에이치엔클라우드’로 설정합니다.

1. RCS Biz Center에서 **기업 대시보드 > 브랜드 대시보드 > 브랜드 운영 관리**로 이동합니다.

2. **대행사 권한 추가**를 클릭한 뒤 대행사명에서 '엔에이치엔클라우드'를 검색해 선택합니다.
...
1. **기업 대시보드 > 브랜드 대시보드 > 템플릿 등록**으로 이동하여, 템플릿을 등록합니다.
    ...
    * 텍스트/이미지 템플릿에 한해서 등록이 가능합니다. 아래 **지원하는 발송 유형** 항목을 참고하세요.
    * RCS 담당자가 승인하며 템플릿 승인까지 2 영업일 정도 소요됩니다.
...
**Notification Hub** > **발신 정보** > **브랜드 관리** 탭에서 연동이 가능합니다. 연동 후에 변경 사항이 있으면 **+ 브랜드 연동** 버튼을 누르면 동기화가 진행됩니다.

<a id="introduction-to-integrated-rcs"></a>

## 통합 RCS 소개

통합 RCS는 단말 제조사 상관없이 국내 이통 3사에서 제공하는 RCS 표준 규격입니다.

<a id="android-rcs-vs-integrated-rcs"></a>

### 안드로이드 RCS vs 통합 RCS

| 구분 | (기존) 안드로이드 RCS 유형 | 통합 RCS 유형 |
|---|---|---|
| 수신 가능 단말 | 안드로이드 채팅+ 지원 단말 (ex. 삼성 갤럭시) | 국내 이통사와 RCS 연동된 모든 단말 |
| (광고) 표기 | message API 내 "header" 설정하여 메시지 발송 | 메시지 타이틀 혹은 본문에 직접 표기하여 메시지 발송 (글자수에 포함) |
| 무료수신거부 표기 | message API 내 "footer" 설정하여 메시지 발송 | 메시지 본문 끝에 직접 표기하여 메시지 발송 (글자수에 포함) |
| CopyAllowed | 지원 (메시지별 사용자 복사 가능여부 설정 가능) | 미지원 (단말 정책에 따라 복사 기능 제공됨) |
...
### 참고사항
- 2025년 8월 기준 사용중인 Biz RCS 유형은 모두 안드로이드 RCS 유형으로 정의합니다.
- 기존에 사용하던 안드로이드 RCS 유형의 메시지도 계속 이용 가능합니다.
