...
!!! danger "주의"
    엔진 버전을 변경하면 다음과 같이 HTTP 트래픽 처리 동작이 달라질 수 있습니다. 운영 환경에 적용하기 전에 반드시 테스트하시기 바랍니다.
    ...
    * HTTP 표준 준수: `v2`는 HTTP 표준을 더 엄격하게 준수합니다. 비표준 형식의 요청/응답을 사용하던 경우 동작이 달라질 수 있습니다.
    ...
    `v2`는 HTTP 표준(RFC)을 준수하지만, 그 과정에서 기존(`v1`) 대비 일부 동작이 조금씩 달라질 수 있습니다. 위 항목은 대표적인 예시이며 명시되지 않은 다른 동작도 달라질 수 있으므로, 엔진 버전을 변경한 후에는 반드시 충분히 검증한 뒤 운영 환경에 적용하시기 바랍니다.
