...
이를 이용하면 프로토콜, IP, 포트별로 접근 제어가 가능합니다.
...
* 순서 번호에 따라 우선순위를 가지고 순서대로 적용됩니다. 작은 번호가 높은 우선순위를 가집니다.
* '허용(Allow)': 해당 규칙과 매칭되는 접근은 허용합니다.
...
* 출발지(src)와 목적지(dst)에 대해 규칙을 항상 짝으로 설정해야 합니다.


<a id="4"></a>
### ACL Binding
* 하나의 ACL에 바인딩될 수 있는 네트워크 수는 최대 VPC 개수만큼입니다.
...
> <a id="5"></a>
> ### Network와 ACL 동작
> * ACL에 바인딩된 네트워크가 있으면 ACL을 삭제할 수 없습니다.
> * ACL 규칙을 추가하거나 삭제하면 바인딩된 모든 네트워크에 해당 내역이 반영됩니다.
* ACL 규칙을 추가하거나 삭제하면 바인딩된 모든 네트워크에 해당 내역이 반영됩니다.
