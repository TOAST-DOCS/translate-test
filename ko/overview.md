...
이를 이용하면 프로토콜, IP, 포트 별로 접근 제어가 가능해집니다.

<a id="1"></a>
## ACL 기능

네트워크로 유입되는 패킷을 제어하려면 ACL 기능을 이용할 수 있습니다.
이 기능은 [보안 그룹](/Network/Security%20Groups/ko/overview/)과 구분되는 기능으로, 차이점은 다음과 같습니다.
...
> | 설정 대상 | 프로토콜, IP, 포트 설정 | 프로토콜, IP, 포트 설정 | ACL은 블랙리스트, 화이트리스트 선택적으로 운영 가능 |
...
<a id="3"></a>
### ACL Rule
* 하나의 프로젝트에 최대 100개의 규칙을 생성할 수 있습니다.
* 순서 번호에 따라서 우선순위를 가지고 순서대로 적용됩니다. 작은 번호가 높은 우선순위를 가집니다.
...
* src와 dst에 대해 규칙을 항상 짝으로 설정해야 합니다.


<a id="4"></a>
### ACL Binding
* 하나의 ACL에 바인딩될 수 있는 네트워크의 수는 최대 VPC 개수만큼입니다.
* 하나의 ACL은 여러 네트워크에 적용할 수 있습니다.
* 하나의 네트워크에는 하나의 ACL을 적용할 수 있습니다.

> [참고]
> <a id="5"></a>
> ### 네트워크와 ACL 동작
> * 네트워크를 삭제하면 ACL 바인딩이 삭제됩니다. ACL은 삭제되지 않습니다.
> * ACL에 바인딩된 네트워크이 있으면 ACL을 삭제할 수 없습니다.
> * ACL rule을 추가하거나 삭제하면 바인딩된 모든 네트워크에 해당 내역이 반영됩니다.
