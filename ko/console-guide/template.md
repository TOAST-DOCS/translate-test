...
* 자동 등록
    ...
    * 공용 알림톡 템플릿은 **카카오** 카테고리 아래에 자동으로 분류됩니다. 해당 카테고리는 시스템이 생성하고 관리하므로 사용자가 직접 수정하거나 삭제할 수 없습니다.
* 사용 제약
    * 공용 알림톡 템플릿은 카카오가 관리하므로 콘솔에서 수정하거나 삭제할 수 없습니다.
    * 공용 알림톡 템플릿을 일반 카테고리로 이동하거나, 일반 템플릿을 **카카오** 카테고리로 이동할 수 없습니다.
* 발송
    * 공용 알림톡 템플릿은 특정 발신 프로필에 귀속되지 않으므로, 발송 시 사용할 발신 프로필을 직접 선택해야 합니다.
    * 활성 상태의 발신 프로필이 하나 이상 등록되어 있어야 합니다.
