...
공용 알림톡 템플릿은 카카오에서 직접 제작하고 검수하여 모든 사업자가 공통으로 사용할 수 있도록 공개한 템플릿입니다. 특정 발신 프로필에 귀속되지 않으며, 카카오 검수가 이미 완료된 상태로 제공되므로 별도의 검수 요청 없이 바로 발송에 사용할 수 있습니다.

* 자동 등록
    * 발신 프로필을 등록하면 공용 알림톡 템플릿이 자동으로 동기화되어 콘솔에 노출됩니다. 이후 카카오에 새로운 공용 템플릿이 공개되면 주기적으로 자동 동기화됩니다.
    * 공용 알림톡 템플릿은 **카카오** 카테고리 아래에 자동으로 분류됩니다. 이 카테고리는 시스템이 생성하고 관리하므로 사용자가 직접 수정하거나 삭제할 수 없습니다.
* 사용 제약
    * 공용 알림톡 템플릿은 카카오가 관리하므로 콘솔에서 **수정하거나 삭제할 수 없습니다.**
    * 공용 알림톡 템플릿을 일반 카테고리로 옮기거나, 일반 템플릿을 **카카오** 카테고리로 옮길 수 없습니다.
* 발송
    * 공용 알림톡 템플릿은 특정 발신 프로필에 귀속되지 않으므로, 발송 시 사용할 발신 프로필을 직접 선택해야 합니다. 활성 상태의 발신 프로필이 하나 이상 등록되어 있어야 합니다.
    * 검수가 완료된 상태이므로 검수 대기 없이 바로 발송할 수 있습니다.
