...
1. 발신 프로필을 선택합니다.
2. 템플릿 사용 여부를 선택합니다.
    * 템플릿을 사용하는 경우, 발신 프로필에 등록된 템플릿을 선택합니다.
    * 템플릿을 사용하지 않는 경우, 메시지 타입을 선택하고 내용을 작성합니다.
3. 푸시 알림 여부를 선택합니다.
    * 푸시 알림을 **사용 안 함**으로 선택하면 메시지 푸시 알림이 발송되지 않습니다.
4. 템플릿을 사용하지 않는 경우, 메시지 타입을 선택합니다.
    * **텍스트**: 한/영 구분 없이 띄어쓰기 포함 1,300자 텍스트 + 링크 버튼 최대 5개(세로 배열)
    * **이미지**: 한/영 구분 없이 띄어쓰기 포함 1,300자 텍스트 + 이미지 1장 + 링크 버튼 최대 5개(세로 배열)
    * **와이드 이미지**: 한/영 구분 없이 띄어쓰기 포함 76자 텍스트 + 이미지 1장 + 링크 버튼 최대 2개
    * **와이드 아이템 리스트**: 1개의 타이틀에 3~4개의 리스트(이미지 + 아이템) + 링크 버튼 최대 2개(가로 정렬). 첫 번째 아이템 제목 25자, 2~4번째 아이템 제목 30자.
    * **캐러셀 피드**: 제목 20자 텍스트 + 문구 180자 텍스트 + 이미지 + 링크 버튼 2개(가로 정렬)로 구성된 아이템 최대 6개
    * **프리미엄 동영상**: 카카오TV에 업로드된 영상 + 헤더 20자 텍스트 + 문구 76자 텍스트 + 링크 버튼 1개
    * **커머스**: 상품 가격 및 할인 정보를 강조하는 타입. 제목 20자 텍스트 + 부가 정보 34자 텍스트 + 링크 버튼 최대 2개(가로 정렬)
    * **캐러셀 커머스**: 상품 정보를 카탈로그 형태로 구성하는 타입. 제목 30자 텍스트 + 부가 정보 34자 텍스트 + 링크 버튼 최대 2개(가로 정렬)로 구성된 아이템 최대 6개. 모든 이미지는 동일한 비율이어야 합니다.
5. 이미지가 있는 경우 이미지를 선택합니다. 이미지를 첨부하려면 먼저 이미지를 등록해야 합니다.
6. 버튼을 삽입할 수 있습니다.
7. 발송 대상 유형을 선택합니다.
    * **고객 대상 발송**: 광고주의 마케팅 수신 동의(마수동) 사용자를 대상으로 발송합니다. 080 수신거부번호 등록이 필요합니다.
    * **친구 대상 발송**: 카카오톡 채널 친구 전체를 대상으로 발송합니다.
8. 고객 대상 발송인 경우, 타겟팅 유형을 선택합니다.
    * **M**: 마케팅 수신 동의 사용자 전체를 대상으로 광고성 메시지를 발송합니다.
    * **N**: 마케팅 수신 동의 사용자 중 채널 친구를 제외하고 광고성 메시지를 발송합니다.
    * **O**: 마케팅 수신 동의 사용자 중 채널 친구를 대상으로 광고성 메시지를 발송합니다.
9. 고객 대상 발송인 경우, 080 수신거부번호를 설정합니다.
    * 발신 프로필에 설정된 080 수신거부번호를 사용하거나, 직접 입력할 수 있습니다.
10. 대체 발송 메시지 설정 여부를 선택합니다.
    * 브랜드 메시지 발송 실패 시 문자 메시지로 대체 발송할 수 있습니다.
11. 수신자를 추가합니다.
12. **발송**을 클릭해 메시지를 발송합니다.

##### 브랜드 메시지 버튼 유형

| 버튼 종류 | 설명 |
| --- | --- |
| 웹 링크 | 모바일 또는 PC 웹페이지로 이동합니다. |
| 앱 링크 | 커스텀 스킴으로 앱을 실행합니다. Android와 iOS 스킴을 각각 설정합니다. |
| 봇 키워드 | 버튼 이름이 상담원에게 전달됩니다. |
| 메시지 전달 | 버튼 이름, 메시지 본문이 상담원에게 전달됩니다. |
| 상담톡 전환 | 상담톡에 연결됩니다. |
| 봇 전환 | 챗봇이 호출됩니다. |
