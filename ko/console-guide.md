...
## Network > Load Balancer > 콘솔 사용 가이드 { #network-load-balancer-console-guide }

<a id='manage-loadbalancers'></a>
## 로드 밸런서 관리 { #manage-loadbalancers }

<a id='create-loadbalancers'></a>
### 로드 밸런서 생성 { #create-loadbalancers }
...
* Network (Subnet): 로드 밸런서가 연결될 VPC의 서브넷을 지정합니다. **자동 할당**을 선택하면 서브넷 범위 내에서 사용 가능한 사설 IP를 할당받아 로드 밸런서에 부여합니다. **지정**을 선택해 원하는 사설 IP를 로드 밸런서에 부여할 수 있습니다. 
...
* Keep-Alive 타임아웃: 클라이언트 및 서버와의 세션 유지 시간을 초 단위로 지정합니다. 로드 밸런서는 상대편이 세션을 유지하는 한 이 시간 동안 세션을 유지합니다. 서버에 설정한 Keep-Alive 타임아웃 값을 이곳에 설정하는 것을 추천합니다. 기본값은 300초로 설정됩니다.
...
* SSL 정책: 프로토콜로 TERMINATED_HTTPS를 선택한 경우, 리스너에 연결할 사용자 정의 SSL 정책을 선택할 수 있습니다.
  * **사용 안 함**을 선택하면 리스너의 TLS 버전 설정에 따라 기본 제공되는 암호화 스위트가 적용됩니다.
  * SSL 정책을 선택하면 해당 정책에 정의된 암호화 스위트가 적용되며, 리스너의 TLS 버전은 해당 정책의 최소 TLS 버전과 동일하게 설정해야 합니다.
  * 사용할 SSL 정책이 없는 경우 **SSL 정책 관리** 메뉴에서 먼저 정책을 생성해야 합니다.

!!! danger "주의"
    - 로드 밸런서 포트 및 프로토콜은 리스너 생성 후 변경이 불가능합니다.
    - 리스너의 기본 멤버 그룹을 **사용 안 함**으로 지정할 수 있습니다. 만약 L7 규칙이 없거나 혹은 있더라도 조건에 부합하지 않아 **사용 안 함**으로 로드 밸런싱을 결정하게 된다면, 해당 요청은 503을 반환합니다.

!!! tip "알아두기"
    - 로드 밸런서 포트는 1부터 65535 사이의 값을 가집니다.
    - 멤버 그룹이 리스너의 기본 멤버 그룹으로 속해 있거나 L7 규칙의 작업 대상으로 지정된 경우에만 상태 확인을 수행합니다. 그렇지 않으면 해당 멤버 그룹으로는 상태 확인을 수행하지 않습니다.

!!! tip "알아두기"
    TERMINATED_HTTPS 인증서 등록 방법
...
!!! danger "주의"
    - 조건 유형 중에서 호스트명은 대소문자를 구분하지 않습니다.
    - 리다이렉트 URL을 올바르지 않은 형태로 입력 시, 리다이렉트 URL이 실제 입력과는 다른 값으로 변환될 수 있습니다.

!!! tip "알아두기"
    - 설정한 L7 규칙에 매칭되지 않을 경우 리스너의 기본 멤버 그룹으로 트래픽이 전달됩니다.
    - 멤버 그룹이 리스너의 기본 멤버 그룹으로 속해 있거나 L7 규칙의 작업 대상으로 지정된 경우에만 상태 확인을 수행합니다. 그렇지 않으면 해당 멤버 그룹으로는 상태 확인을 수행하지 않습니다.
    - 멤버 그룹이 삭제될 경우, 해당 멤버 그룹을 작업 대상으로 갖고 있던 L7 규칙들은 작업 유형이 차단으로 변경됩니다.
    - L7 규칙에서 **URL로 전달**로 설정 시 리다이렉트하려는 URL의 일부 값을 유지하거나 직접 지정할 수 있습니다. 만약 특정 필드의 값을 유지하고 싶은 경우 각 URI 부분 설정 필드에 `#{protocol}`, `#{port}`, `#{host}`, `#{path}`, `#{query}`와 같은 형태로 입력합니다. 예를 들어, HTTP로 유입된 요청에 대해서 프로토콜과 포트번호만 HTTPS 및 443 포트로 변경하고 싶다면 프로토콜은 `HTTPS`로, 포트는 `443`으로, 호스트는 `#{host}`, 경로는 `#{path}`로, 쿼리는 `#{query}`로 입력합니다.
...
* 프로토콜: 멤버 그룹이 처리할 트래픽의 프로토콜을 지정합니다. HTTP/HTTPS/TCP/HTTP_REENCRYPT 중 하나를 선택합니다.
...
!!! danger "주의"
    멤버 포트 및 프로토콜은 멤버 그룹 생성 후 변경이 불가능합니다.

!!! tip "알아두기"
    멤버 포트는 1부터 65535 사이의 값을 가집니다.
...
* 상태 확인 프로토콜: 상태 확인 시 사용할 프로토콜을 결정합니다. TCP/HTTP/HTTPS 중 하나를 선택합니다.
...
* HTTP 메서드: 상태 확인 시 사용할 HTTP 메서드를 선택합니다. 상태 확인 프로토콜로 HTTP나 HTTPS를 선택했을 때만 활성화됩니다. 현재는 GET만을 지원합니다.
* HTTP 상태 코드: 상태 확인 시 정상 응답으로 간주할 HTTP 상태 코드를 입력합니다. 상태 확인 프로토콜로 HTTP나 HTTPS를 선택했을 때만 활성화됩니다. 현재는 GET만을 지원합니다.
...
### 로드 밸런서 살펴보기 { #view-loadbalancers }
로드 밸런서 생성을 마치면 다시 로드 밸런서 목록 화면으로 돌아오게 됩니다. 로드 밸런서 목록 화면에서는 생성된 로드 밸런서들의 기본 정보를 확인할 수 있습니다. 목록 화면에서 노출되는 항목들은 다음과 같습니다.

* 이름: 로드 밸런서 생성 시 지정한 로드 밸런서의 이름입니다.
* 타입: 로드 밸런서 타입입니다.
* IP 주소: 로드 밸런서에 연결된 VPC로부터 할당받은 사설 IP입니다. VPC 내부에서는 이 IP를 통해 로드 밸런서로 접근할 수 있습니다. VPC 외부에서 로드 밸런서에 접근하려면 플로팅 IP를 연결해야 합니다.
* 네트워크: 로드 밸런서에 연결된 VPC의 이름과 서브넷 CIDR입니다.
* 상태: 로드 밸런서 생성 상태를 나타냅니다. ACTIVE로 표기된다면 정상적으로 생성된 것입니다.
* 로드 밸런서 상세: 로드 밸런서에 연결된 리스너와 멤버 그룹의 상세 내용을 확인할 수 있습니다.

!!! tip "알아두기"
    로드 밸런서의 생성 상태는 다음 중 하나로 결정됩니다.
...
### 로드 밸런서 수정 및 기본 정보 { #modify-loadbalancers }
...
!!! tip "알아두기"
    로드 밸런서가 연결된 VPC와 IP 주소는 변경할 수 없습니다.

<a id='change-listener'></a>
### 리스너 변경 및 상세 정보 { #change-listener }
로드 밸런서의 기본 화면에서 원하는 로드 밸런서의 상세 보기를 선택하면 로드 밸런서에 연결된 리스너와 멤버 그룹을 확인할 수 있습니다. 이 중 **리스너** 탭을 선택해 리스너를 생성, 변경, 삭제할 수 있습니다.

<a id='change-listener-add-listeners'></a>
#### 리스너 생성
**리스너 생성**을 클릭하면 리스너를 추가로 생성할 수 있습니다. 리스너 추가에 필요한 항목들은 로드 밸런서 생성 시 기본 리스너에서 필요한 항목들과 동일합니다. 리스너 추가 시 기존에 존재하던 리스너들이 사용하던 로드 밸런서 포트는 사용할 수 없습니다.
...
!!! danger "주의"
    인증서 추가 시 로드 밸런서가 재기동됩니다. 재기동하는 과정에서 기존에 연결된 세션은 유지되나, 새로운 세션은 처리할 수 없습니다(약 1초 미만). 따라서, 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장합니다.
...
!!! danger "주의"
    기본 인증서 변경 시 로드 밸런서가 재기동됩니다. 재기동하는 과정에서 기존에 연결된 세션은 유지되나, 새로운 세션은 처리할 수 없습니다(약 1초 미만). 따라서, 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장합니다.
...
!!! danger "주의"
    - 기본 인증서는 삭제할 수 없습니다. 기본 인증서를 삭제하려면 먼저 다른 인증서를 기본 인증서로 변경한 후 삭제해야 합니다.
    - 인증서 삭제 시 로드 밸런서가 재기동됩니다. 재기동하는 과정에서 기존에 연결된 세션은 유지되나, 새로운 세션은 처리할 수 없습니다(약 1초 미만). 따라서, 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장합니다.

<a id='custom-response-guide'></a>
### 사용자 정의 응답 가이드 { #custom-response-guide }
...
- 삭제된 응답에 해당하는 오류 코드 발생 시 기본 시스템 응답이 노출됩니다.

!!! tip "알아두기"
    동일한 리스너 내에서 각 오류 코드는 한 번만 사용자 정의 응답으로 등록할 수 있습니다.

!!! danger "주의"
    사용자 정의 응답을 추가, 수정 또는 삭제할 때 리스너가 잠시(1초 미만) 재기동될 수 있으므로, 서비스 영향이 적은 시간에 변경 작업을 진행하는 것을 권장합니다.

<a id='x-forwarded-header-guide'></a>
### X-Forwarded 헤더 설정 가이드 { #x-forwarded-header-guide }
...
!!! danger "주의"
    X-Forwarded 헤더 설정을 변경하면 로드 밸런서가 재기동됩니다. 재기동하는 과정에서 기존에 연결된 세션은 유지되나, 새로운 세션은 처리할 수 없습니다(약 1초 미만). 따라서, 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장합니다.
...
!!! danger "주의"
    리스너 생성/수정/삭제 시 로드 밸런서가 재기동됩니다. 재기동하는 과정에서 기존에 연결된 세션은 유지되나, 새로운 세션은 처리할 수 없습니다(약 1초 미만). 따라서, 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장합니다.

<a id='change-member-group'></a>
### 멤버 그룹 변경 및 상세 정보 { #change-member-group }
로드 밸런서 화면에서 원하는 로드 밸런서의 **상세 보기**를 선택하면 로드 밸런서에 연결된 리스너와 멤버 그룹을 확인할 수 있습니다. 이 중 **멤버 그룹** 탭을 선택해 멤버 그룹을 생성, 변경, 삭제할 수 있습니다.
...
#### 멤버 그룹 삭제
삭제할 멤버 그룹을 선택하고 **멤버 그룹 삭제**를 클릭하면 해당 멤버 그룹은 삭제됩니다.

!!! danger "주의"
    멤버 그룹 생성/수정/삭제 시 로드 밸런서가 재기동됩니다. 재기동하는 과정에서 기존에 연결된 세션은 유지되나, 새로운 세션은 처리할 수 없습니다(약 1초 미만). 따라서, 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장합니다.

!!! tip "알아두기"
    멤버 그룹이 삭제될 경우, 해당 멤버 그룹을 작업 대상으로 갖고 있던 L7 규칙들은 작업 유형이 차단으로 변경됩니다.

<a id='change-member'></a>
### 멤버 변경 및 상세 정보 { #change-member }
...
!!! danger "주의"
    멤버 추가/비활성화/삭제 시 로드 밸런서가 재기동됩니다. 재기동하는 과정에서 기존에 연결된 세션은 유지되나, 새로운 세션은 처리할 수 없습니다(약 1초 미만). 따라서, 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장합니다.

<a id='delete-loadbalancers'></a>
### 로드 밸런서 삭제 { #delete-loadbalancers }
로드 밸런서의 목록 화면에서 삭제하려는 로드 밸런서를 선택한 후 **삭제** 버튼을 클릭하면, 해당 로드 밸런서가 삭제됩니다.
...
#### IP 접근제어 그룹 생성
IP 접근제어 그룹을 생성하려면 **접근제어 그룹 생성** 버튼을 클릭하고, 다음의 값을 입력합니다.

* 이름: 접근제어 그룹의 이름을 입력합니다.
* 설명: 접근제어 그룹의 설명을 기술합니다.
* IP 접근제어 타입: 차단과 허용 중 선택합니다.
* IP 접근제어 대상 추가: 접근제어 대상이 되는 IP와 설명을 기술합니다. 우측 **+** 버튼을 클릭하여 여러 개의 접근제어 대상을 한꺼번에 추가할 수 있습니다. 대량 입력을 좀더 수월하게 하려면 **대량 입력**을 선택할 수 있습니다. 이 경우 한 줄에 "IP주소 또는 CIDR"와 "설명"을 입력합니다. 한 번에 최대 100개의 접근제어 대상을 입력할 수 있습니다.


**확인**을 클릭하면 접근제어 그룹과 대상이 생성됩니다.

!!! tip "알아두기"
    IP 접근제어 그룹과 IP 접근제어 대상의 개수

    프로젝트별 접근제어 그룹을 최대 10개까지 생성할 수 있습니다.
    프로젝트별 접근제어 대상을 최대 1,000개까지 생성할 수 있습니다.
...
#### IP 접근제어 그룹 적용
IP 접근제어 그룹을 적용할 로드 밸런서를 선택합니다. 해당 로드 밸런서에 설정할 그룹을 선택하고 **확인**을 클릭합니다.
"접근제어 타입"이 동일한 그룹 여러 개를 로드 밸런서에 적용할 수 있습니다.

<a id='ssl-policies'></a>
## SSL 정책 관리 { #ssl-policies }
SSL 정책은 리스너에서 사용할 최소 TLS 버전과 암호화 스위트의 조합을 정의한 사용자 정의 보안 정책입니다. SSL 정책의 개념과 선택 가능한 암호화 스위트 목록은 [사용자 정의 SSL 정책](/Network/Load%20Balancer/ko/overview/#ssl) 문서를 참고합니다.

<a id='create-ssl-policies'></a>
#### SSL 정책 생성
SSL 정책을 생성하려면 **SSL 정책 생성** 버튼을 클릭하고, 다음의 값을 입력합니다.

* 이름: SSL 정책의 이름을 입력합니다.
* 최소 SSL/TLS 버전: SSL 정책이 허용하는 최소 SSL/TLS 버전을 선택합니다. `SSLv3`, `TLSv1.0`, `TLSv1.0_2016`, `TLSv1.1`, `TLSv1.2`, `TLSv1.3` 중 하나를 선택합니다.
* 암호화 스위트 목록: 선택한 최소 SSL/TLS 버전에서 사용 가능한 암호화 스위트가 표시됩니다. 사용할 스위트를 선택합니다.
* 설명: SSL 정책에 대한 설명을 기술합니다.

!!! danger "주의"
    - 최소 SSL/TLS 버전은 생성 후 변경할 수 없습니다. 변경이 필요하면 새 SSL 정책을 생성하여 리스너에 적용해야 합니다.
    - 암호화 스위트는 최소 하나 이상 선택해야 합니다.

!!! tip "알아두기"
    SSL 정책은 프로젝트당 최대 10개까지 생성할 수 있습니다.

<a id='change-ssl-policies'></a>
#### SSL 정책 변경
SSL 정책의 이름, 설명, 암호화 스위트를 변경할 수 있습니다. 최소 SSL/TLS 버전은 변경할 수 없습니다.

SSL 정책의 암호화 스위트를 변경하면 해당 정책이 적용된 모든 리스너의 설정이 자동으로 갱신됩니다.

!!! danger "주의"
    SSL 정책 변경 시 해당 정책이 연결된 리스너의 로드 밸런서가 재기동됩니다. 재기동 과정에서 기존에 연결된 세션은 유지되지만, 새로운 세션은 최대 1초 이하 동안 처리되지 않을 수 있습니다. 서비스에 영향을 주지 않는 시간에 진행하는 것을 권장하며, 변경 화면에서 안내 체크박스를 선택해야 변경이 진행됩니다.

<a id='delete-ssl-policies'></a>
#### SSL 정책 삭제
SSL 정책을 삭제할 수 있습니다. 단, 하나 이상의 리스너에 연결된 SSL 정책은 삭제할 수 없습니다. 먼저 리스너의 SSL 정책을 **사용 안 함**으로 변경하여 연결을 모두 해제한 뒤 삭제해야 합니다.

<a id='apply-ssl-policies'></a>
#### SSL 정책 적용
SSL 정책은 리스너 생성 화면 또는 [리스너 변경 및 상세 정보](#change-listener) 화면에서 리스너에 연결합니다.

* 리스너의 프로토콜이 TERMINATED_HTTPS인 경우에만 SSL 정책을 연결할 수 있습니다.
* 리스너에 SSL 정책을 연결할 경우, 리스너의 TLS 버전은 선택한 정책의 최소 TLS 버전과 일치해야 합니다.
* 연결을 해제하려면 리스너 변경 화면에서 SSL 정책을 **사용 안 함**으로 선택합니다.

<a id='restarting-guide-for-maintenance'></a>
## 장비 점검을 위한 로드 밸런서 재시작 가이드 { #restarting-guide-for-maintenance }
...
1. 점검 대상 로드 밸런서를 확인합니다. 로드 밸런서 이름 옆에 **! 재시작** 버튼이 있는 로드 밸런서가 점검 대상 로드 밸런서입니다.
   ...
   **! 재시작** 버튼 위에 마우스 포인터를 올리면 자세한 점검 일정을 확인할 수 있습니다. 서비스에 영향을 주지 않는 시간에 수행하시기를 바랍니다.
