...
| sort_key | Query | String | N | 조회할 ACL의 정렬 키<br>`sort_dir`에서 지정한 방향대로 정렬 |
...
| acls.created_at | Body | String | ACL 생성 시간 |
| acls.updated_at | Body | String | ACL 갱신 시간 |
...
| acl\_rules.policy | Body | String | `allow` 또는 `deny`  |
...
| acl\_rule.ethertype | Body | String | IPv4 고정 |
...
| acl\_rule.tenant_id | Body | String | - | 테넌트 ID |
| acl\_rule.description | Body | String | N | ACL Rule 설명 |
| acl\_rule.acl_id | Body | String | Y | ACL ID |
| acl\_rule.protocol | Body | String | N | Protocol(tcp, udp, icmp) |
| acl\_rule.ethertype | Body | String | N | IPv4로 고정 |
| acl\_rule.src\_port\_range\_min | Body | Integer | N | src 포트 범위의 최솟값(1~65535)|
| acl\_rule.src\_port\_range\_max | Body | Integer | N | src 포트 범위의 최댓값(1~65535)|
| acl\_rule.src_ip | Body | String | Y | src ip |
| acl\_rule.dst_ip | Body | String | Y | dst ip |
| acl\_rule.dst\_port\_range\_max | Body | Integer | N | dst 포트 범위의 최솟값(1~65535)|
| acl\_rule.dst\_port\_range\_min | Body | Integer | N | dst 포트 범위의 최댓값(1~65535)|
| acl\_rule.policy | Body | String | Y | allow or deny  |
| acl\_rule.order | Body | Integer | Y | ACL Rule 적용순서, 숫자가 작을수록 먼저 적용됨(102~32764 사용).|
...
| 이름 | 종류 | 형식 | 필수 | 설명 |
|---|---|---|---|---|
| aclRuleId | URL | UUID | Y | 삭제할 ACL Rule ID |
| tokenId | Header | String | Y | 토큰 ID |
