...
- 조회 결과가 너무 크면 실행이 거절됩니다. 조건을 좁히거나 행 제한을 줄여 다시 실행합니다.
...
| 값 | 설명 |
| --- | --- |
| 확인 전 | 반영 여부를 아직 확인하지 않음. 앱을 만든 직후에는 자동으로 확인되어 반영됨으로 시작합니다 |
| 반영 중 | 변경한 설정을 반영하는 중 |
| 반영됨 | 변경한 설정이 반영 완료 |
...
| 학습 실행 | 학습을 즉시 실행 |
| 자동 재학습 중지 | 자동 재학습 중지 |
| 자동 재학습 재개 | 중지한 자동 재학습 재개. 학습 주기를 지정하지 않았으면 먼저 주기를 설정해야 합니다 |
| 새로고침 | 학습 상태 다시 조회 |
...
- **오류**는 추론이 실패해 결과가 나가지 않는 그룹 수입니다. 앞 세 칸과 다른 축이라 합계에 더해지지 않으며, 켜져 있는 오류 그룹은 활성화에도 함께 세어집니다.
...
| 상태 | 그룹의 현재 상태 |
| 추론 상태 | 이 그룹의 추론이 정상으로 돌고 있는지. 상태 칼럼과는 다른 축 |
...
- 추론 상태는 상태 칼럼과 다른 축입니다. 꺼 둔 그룹에도 오류 기록이 남아 있을 수 있고, 오류인 그룹도 켜져 있으면 활성화 상태로 표시됩니다.
...
- 상태와 추론 상태로 각각 필터링하거나 그룹 키 또는 그룹 해시로 검색해 목록을 좁힐 수 있습니다. 두 필터는 축이 달라 함께 걸 수 있습니다.
