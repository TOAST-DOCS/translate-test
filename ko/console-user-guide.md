...
서비스 활성화는 [1:1 문의](https://www.nhncloud.com/kr/support/inquiry)로만 신청할 수 있습니다. 원하는 리소스 크기를 포함하여 문의하세요. 신청한 클러스터가 생성되면 데이터 소스 생성부터 순서대로 사용할 수 있습니다.
...
| 단변량 시계열 이상탐지 결과 | 단변량 시계열 이상탐지 결과가 저장되는 데이터 소스 |
...
- 기본 키를 지정하지 않으면 Event API로는 데이터 추가만 할 수 있습니다. 기존 행을 수정하거나 삭제하는 이벤트는 거절됩니다.
...
- 그룹 라벨: 예를 들어 규칙 ID 라벨을 지정하면 규칙 값마다 그룹이 하나씩 생깁니다. 비워 두면 데이터 소스 전체가 하나의 그룹이 됩니다. 비워 둔 경우에는 앱 생성이 끝날 때 그 그룹 하나가 등록되고, 라벨을 지정한 경우에는 API로 그룹을 등록해야 합니다.
...
그룹 라벨 아래 **예시로 보기** 행에는 입력한 라벨 설정으로 예시 지표가 몇 개의 시리즈와 그룹으로 나뉘는지 '시리즈 N개·그룹 M개' 형식으로 표시됩니다. 그룹 라벨을 비우면 '그룹 1개·데이터 소스 전체가 한 그룹입니다'로 표시됩니다.

- **예시 보기**를 클릭하면 그룹별로 시리즈를 나눈 표와 요청 본문 예시가 펼쳐집니다. 표의 줄을 클릭하면 요청 본문 예시가 그 시리즈의 라벨로 바뀝니다.
- 시리즈 식별 라벨을 **직접 지정**해 여러 지표가 한 시리즈로 합쳐지면 합쳐진 개수가 함께 표시됩니다.
- 입력을 바꾸면 즉시 다시 계산됩니다.
...
- 데이터 적재 작업이 진행 중이거나 데이터 소스에 다른 작업이 진행 중이면 삭제할 수 없습니다. 작업이 끝난 뒤 다시 시도합니다.
...
!!! danger "주의"
    Event API를 활성화하면 CSV 파일 업로드와 Ingest API 스냅숏 업로드가 차단됩니다. 스냅숏을 업로드하려면 Event API를 먼저 비활성화해야 합니다.
    활성화 상태에서는 데이터 소스 스키마 변경도 제한됩니다. 카탈로그에 필드를 추가하려면 Event API를 먼저 비활성화해야 합니다.
    스냅숏 업로드 후 Event API를 다시 활성화하면 이벤트 수집 위치가 최신 오프셋으로 초기화됩니다.
...
| 요청 헤더 | 이 데이터 소스의 앱 키가 채워진 `X-NC-APP-KEY` 헤더와 인증 토큰을 넣는 `X-NHN-Authorization` 헤더 |
| 요청 본문 예시 | 이 데이터 소스의 그룹 라벨이 반영된 요청 본문 예시 |
| 규칙 | 인증 토큰, 시각 단위, 전송 간격, 라벨, 부가 정보, 값, 응답 관련 유의 사항 |

같은 내용은 데이터 소스를 만든 직후 완료 창에도 표시됩니다.
...
비교할 값은 **직접 입력**, **시간(현재 기준)**, **다른 칼럼**, **함수·식** 중에서 선택합니다.

- **함수·식**에는 상수 식을 입력합니다. 밀리초 epoch로 저장된 시간 칼럼을 현재 시각 기준으로 거를 때 사용합니다. 예: `CAST(UNIX_TIMESTAMP(NOW() - INTERVAL 7 DAY) * 1000 AS BIGINT)`
- 함수·식에는 시간 계산·형 변환 함수, CAST 대상 타입, INTERVAL 단위만 사용할 수 있습니다. 영문자, 숫자, 공백과 `_ ( ) + - * / , . :` 만 입력할 수 있고 최대 500자입니다.
- 칼럼을 비교하려면 값 종류를 **다른 칼럼**으로 바꿉니다. 함수·식에 칼럼 이름을 넣을 수 없습니다.
- 허용하지 않는 값을 입력하면 입력란 아래에 사유가 바로 표시되고 저장 버튼이 잠깁니다. SQL 주석도 넣을 수 없습니다.
...
- 쿼리 실행에 실패하면 실패 원인이 결과 영역에 표시됩니다. SQL 문법이 잘못된 경우에는 쿼리 엔진이 알려 준 위치와 내용을 그대로 보여 줍니다.
- 조회 결과가 너무 크거나, 처리에 필요한 메모리를 넘거나, 실행 시간이 초과되면 실행이 거절됩니다. 조건을 좁히거나 조인과 정렬을 줄이거나 행 제한을 낮춰 다시 실행합니다.
- 그 밖의 오류는 상세 내용 대신 오류 ID가 표시됩니다. 문의할 때 이 값을 함께 알려 주면 조치가 빠릅니다.
...
AI 모델을 데이터에 연결한 앱을 생성하고 관리합니다. 앱 유형은 **추천 시스템**과 **단변량 시계열 이상탐지** 두 가지를 제공합니다.
...
앱 생성 화면 아래쪽에는 **리소스 확인** 결과가 표시됩니다. 단계와 앱 유형에 관계없이 표시되며, 앱을 만들기 전에 생성 가능 여부를 확인할 수 있습니다.
...
| GPU에 새 모델을 띄울 자리가 없어 재학습 결과를 반영할 수 없습니다. | 디바이스로 GPU를 선택했을 때 표시. 생성 불가. 모델 CPU를 낮추거나 디바이스를 CPU로 바꾸면 진행 가능 |
...
| 콜드스타트 데이터소스 | 이 테이블에 있는 사용자 ID만 콜드스타터로 판정. 데이터 소스와 사용자 ID 칼럼 함께 선택 필요 |
...
- 지표 데이터 소스 하나에는 단변량 시계열 이상탐지 앱을 하나만 만들 수 있습니다. 다른 단변량 시계열 이상탐지 앱이 이미 사용 중인 데이터 소스는 목록에 표시되지 않습니다.
...
- 최근 5분 안에 같은 시계열로 1분에 서로 다른 값이 여러 개 들어왔으면 안내 상자가 표시됩니다. 이 경우에도 앱은 생성할 수 있지만, 매분 먼저 도착한 값 하나만 분석에 쓰이고 나머지는 버려집니다. 같은 시계열은 1분에 한 번만 보내고, 더 짧은 주기로 수집한다면 1분 평균으로 합쳐 보냅니다.
...
- 앱은 지표를 1분 단위로 묶어 시계열마다 판정합니다. 앱에 연결한 데이터 소스에는 같은 시계열을 1분에 하나씩 끊김 없이 보내야 합니다. 그보다 긴 간격으로 보내면 빈 구간이 생겨 정확 모드에서 준비가 끝나지 않을 수 있고, 1분 안에 여러 값을 보내면 먼저 도착한 값만 판정에 쓰입니다. 데이터 소스에 지표를 적재하는 것 자체는 전송 주기와 무관합니다.
- 새로 들어온 지표는 판정에 사용할 데이터가 쌓이고 임계값이 그 지표의 분포로 보정되기까지 보통 6시간 안팎이 걸리며, 지표가 들어오는 양상에 따라 더 걸릴 수 있습니다.
- 데이터 소스에 시계열이 하나뿐이면 학습이 실패합니다. 시계열을 둘 이상 두고, 시계열마다 약 4시간 이상 끊김 없이 지표를 보내야 정상적으로 학습합니다.
- 전송이 몇 분 이상 끊기면 쌓아 둔 구간이 깨져 다시 준비 상태가 됩니다. 정확 모드에서는 다시 채울 때까지 결과가 나가지 않습니다.
- 점수 스케일을 켜면 점수를 0~100 범위로 변환해 내보내고, 임계값도 같은 스케일로 계산합니다. 백분율 축을 사용하는 대시보드와 스케일을 맞출 때 사용합니다. 끄면 원값을 그대로 내보냅니다.
...
- 전송 URL로의 전송이 실패해도 콘솔에는 표시되지 않습니다. 수신 측 Prometheus에 결과가 보이지 않으면 결과 데이터 소스에 저장된 결과와 대조해 확인합니다.

!!! tip "알아두기"
    추론 결과는 Prometheus 전송과 별개로 결과 데이터 소스에 항상 저장됩니다. 결과 데이터 소스는 앱을 생성할 때 자동으로 만들어지며, **분석** 메뉴에서 조회할 수 있습니다.

결과 데이터 소스의 스키마는 다음과 같습니다. **분석** 메뉴의 쿼리나 차트에서 이 칼럼으로 조회합니다.

| 필드 이름 | 자료형 | 설명 |
| --- | --- | --- |
| labelHash | string | 시계열 식별자. 지표 데이터 소스의 labelHash와 같은 값 |
| identityLabels | string | 시계열의 라벨 원문 |
| score_aggregate | double | 이상 점수 |
| threshold | double | 이상 판정 기준값 |
| eventTimestamp | long | 판정 대상 지표의 시각. 밀리초 epoch |
...
- 재학습 주기를 지정하지 않았으면 '지정 안 함(생성 시 1회 학습)'으로 표시됩니다.
- 고정 헤더와 동적 헤더는 입력한 값 대신 '설정됨', '없음' 또는 하이픈으로 표시됩니다.

**저장** 버튼을 클릭하면 앱이 생성됩니다. 성공 시 완료 모달이 표시되고 목록으로 이동하며, 실패 시 오류 메시지가 표시됩니다.

단변량 시계열 이상탐지 앱의 완료 모달에는 앞으로 진행되는 일이 안내됩니다.
...
2. **삭제** 버튼을 클릭합니다.
3. 확인 모달에서 앱 이름을 확인하고 **삭제**를 클릭합니다.
...
앱에 포함된 학습 모델의 학습 주기를 변경하고, 자동 재학습을 중지·재개하거나 학습을 직접 실행합니다. 학습 산출물 이력은 이 탭에서 확인합니다.
...
| 값 | 설명 |
| --- | --- |
| 확인 전 | 반영 여부를 아직 확인하지 않은 상태. 앱을 만든 직후에는 자동으로 확인되어 '반영됨'으로 시작 |
| 반영 중 | 변경한 설정을 반영하는 중 |
| 반영됨 | 변경한 설정이 반영 완료 |
...
| 학습 실행 | 학습을 즉시 실행 |
| 자동 재학습 중지 | 자동 재학습 중지 |
| 자동 재학습 재개 | 중지한 자동 재학습 재개. 학습 주기를 지정하지 않았으면 주기를 먼저 설정해야 함 |
| 새로고침 | 학습 상태 다시 조회 |
...
| 학습 주기가 설정되지 않음 | 학습 주기를 먼저 설정해야 재개할 수 있습니다. |
...
| 학습 상태 | 학습 대기, 학습 중, 학습 완료, 재학습 중지, 학습 실패, 삭제됨 |
| 최근 학습일 | 마지막으로 학습한 시각 |
| 재학습 주기 | '매일 03:00', '매주 월요일 03:00', '6시간마다' 형식. 지정하지 않았으면 '자동 재학습 없음(생성 시 1회 학습)' |
| 전송 모드 | 정확 모드, 즉시 모드 |
...
- 결과 데이터 소스에 저장된 결과는 **분석** 메뉴에서 조회할 수 있습니다.
- 카드 아래에는 그룹 현황이 **전체**, **활성화**, **활성화 대기 중**, **비활성화**, **오류** 다섯 개의 숫자로 표시됩니다. 숫자를 클릭하면 그룹 목록 탭으로 이동하면서 해당 상태로 필터링됩니다.
- **오류**는 추론이 실패해 결과가 나가지 않는 그룹 수입니다. 활성화·활성화 대기 중·비활성화와는 기준이 달라 합계에 더해지지 않으며, 켜져 있는 오류 그룹은 활성화에도 함께 세어집니다.
- 그룹 현황 제목 옆 물음표 아이콘에 마우스를 올리면 세 상태의 뜻을 확인할 수 있습니다. 활성화 대기 중은 정확 모드에서 보통 6시간 안팎이 걸리며, 즉시 모드는 그룹을 켠 직후 활성화됩니다.
- 재학습이 실패하면 학습 상태가 학습 실패로 표시됩니다. 학습할 데이터가 없거나 모자랄 때 생길 수 있으며, 데이터가 쌓이면 다음 재학습에서 다시 시도합니다. 그동안 결과는 직전에 학습한 모델로 계속 나갑니다. 첫 학습이 실패하면 앱이 실패 상태가 되며 삭제할 수 있습니다.
- 학습 상태 라벨 옆 물음표에서 각 값의 뜻을 확인할 수 있습니다. 재학습 중지는 자동 재학습을 꺼 둔 상태이고, 삭제됨은 학습 설정이 지워진 상태입니다.
...
| 상태 | 그룹의 현재 상태 |
| 추론 상태 | 이 그룹의 추론이 정상으로 돌고 있는지. 상태 칼럼과는 별개로 표시. 값에 마우스를 올리면 판정 시각 표시 |
| 탐지 시작 시각 | 판정에 쓸 데이터가 충분히 모여 결과가 나가기 시작한 시각. 활성화 대기 중인 그룹은 하이픈 |
| 비활성화 시각 | 마지막으로 비활성화한 시각. 다시 켜도 지워지지 않아 직전 이력이 남으며, 비활성화한 적이 없으면 하이픈 |
| 생성일 | 그룹이 등록된 일시 |
...
| 값 | 설명 |
| --- | --- |
| 정상 | 탐지가 시작됐고 오류로 보고된 적이 없음 |
| 오류 | 추론이 실패했거나 준비 시간이 초과됨. 그동안 결과가 나가지 않음 |
| 판정 없음 | 아직 탐지가 시작되지 않았거나 꺼 둔 그룹 |

- 한 그룹에는 여러 시계열이 들어갑니다. 그중 하나라도 추론이 멈추면 그룹 전체가 오류가 되고, 그 시계열이 회복되어야 정상으로 돌아옵니다.
- 오류는 두 시점에 판정됩니다. 그룹에 지표가 처음 들어온 뒤 30분이 지나도록 탐지 결과가 한 번도 나오지 않으면 준비 시간이 초과된 것으로 봅니다. 탐지가 돌던 그룹은 지표가 계속 들어오는데도 결과가 10분 넘게 나오지 않으면 추론이 실패한 것으로 봅니다.
- 추론 상태는 상태 칼럼과 별개로 결정됩니다. 꺼 둔 그룹에도 오류 기록이 남아 있을 수 있고, 오류인 그룹도 켜져 있으면 활성화 상태로 표시됩니다.
- 지표가 들어오지 않는 것 자체는 오류로 보지 않습니다. 오류인 시계열은 지표가 10분 넘게 끊기면 판정 대상에서 빠지므로 오류가 아니라 정상으로 자동 복구됩니다.
- 추론 상태 값에 마우스를 올리면 판정 시각이 표시됩니다. 판정 기록이 없으면 시각을 알 수 없다고 표시됩니다.

- 데이터 소스에 그룹 라벨을 지정하면 그 값마다 그룹이 하나씩 생깁니다. 지정하지 않으면 데이터 소스 전체가 하나의 그룹이 됩니다.
- 데이터 소스에 그룹 라벨을 지정하지 않았으면 앱 생성이 끝날 때 그룹 하나가 등록됩니다. 앱을 만드는 중에는 목록이 비어 있고, 생성이 끝나면 나타납니다.
- 그룹 라벨을 지정했으면 그룹이 저절로 등록되지 않습니다. [API 가이드](./api-guide/#univariate-group-api)의 '그룹 사용 시작·중지·삭제'로 대상 그룹을 등록해야 목록에 나타납니다.
- 활성화 대기 중은 정확 모드에서 보통 6시간 안팎이 걸립니다. 즉시 모드는 그룹을 켠 직후 활성화됩니다.
- 오류는 그룹 단위로 판정됩니다. 그룹 안의 시계열 하나만 추론이 멈춰도 그룹 전체가 오류가 되고, 그 시계열이 회복되어야 정상으로 돌아옵니다.
- 상태와 추론 상태로 각각 필터링하거나 그룹 키 또는 그룹 해시로 검색해 목록을 좁힐 수 있습니다. 두 필터는 서로 별개이므로 함께 걸 수 있습니다.
- 추론 상태 필터는 정상, 오류, 판정 없음 세 값을 제공합니다. 세 값은 서로 겹치지 않으며 합치면 전체가 됩니다.
- 툴바의 **새로고침** 버튼으로 목록을 다시 조회할 수 있습니다.
...
- 특정 그룹만 골라 사용 시작, 중지, 삭제하는 기능은 API로만 제공합니다. [API 가이드](./api-guide/#univariate-group-api)의 '그룹 사용 시작·중지·삭제'를 참고합니다.
