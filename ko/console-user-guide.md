...
!!! danger "주의"
    ...
    이 안내는 콘솔에 진입할 때 팝업으로도 표시됩니다. **다시 보지 않기**를 선택하면 같은 브라우저에서는 다시 표시되지 않으며, 다른 브라우저나 다른 프로젝트에서는 다시 표시됩니다.
...
- 그룹 라벨: 예를 들어 규칙 ID 라벨을 지정하면 규칙 값마다 그룹이 하나씩 생깁니다. 비워 두면 데이터 소스 전체가 하나의 그룹이 됩니다. 그룹은 지표가 들어와야 등록됩니다.
...
- 함수·식에는 시간 계산·형 변환 함수, CAST 대상 타입, INTERVAL 단위만 사용할 수 있습니다. 칼럼 참조, 따옴표, 세미콜론, 주석, 500자 초과는 저장할 때 거부됩니다.
...
단변량 이상 탐지 앱의 완료 모달에는 앞으로 진행되는 일이 안내됩니다.

- 모델을 학습하고 배포합니다. 진행 상황은 앱 목록의 상태로 확인합니다.
- 정확 모드에서는 활성화된 뒤에도 판정에 쓸 지표가 쌓여야 결과가 나가기 시작하며, 보통 수 시간이 걸립니다.
- 그동안 지표를 계속 보내야 합니다.
...
2. **삭제** 버튼을 클릭합니다.
3. 확인 모달에서 앱 이름을 확인하고 **확인**을 클릭합니다.
...
- 그룹 현황 제목 옆 물음표 아이콘에 마우스를 올리면 세 상태의 뜻을 확인할 수 있습니다. 활성화 대기 중은 정확 모드에서 보통 수 시간이 걸리며, 즉시 모드는 그룹을 켠 직후 활성화됩니다.
...
| 상태 | 그룹의 현재 상태 |
| 활성화 시각 | 판정에 쓸 데이터가 충분히 모여 결과가 나가기 시작한 시각. 활성화 대기 중인 그룹은 하이픈 |
...
- 그룹은 앱을 만들 때가 아니라 지표가 들어와야 등록됩니다. 앱을 만든 직후에는 목록이 비어 있습니다.
- 활성화 대기 중은 정확 모드에서 보통 수 시간이 걸립니다. 즉시 모드는 그룹을 켠 직후 활성화됩니다.
