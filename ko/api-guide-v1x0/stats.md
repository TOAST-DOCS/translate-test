...
| statsType | Query | Enum | X | 통계 유형<br> - MINUTELY: 0분 ~ 59분 사이 그룹핑<br> - HOURLY: 0시 ~ 23시 사이 그룹핑<br> - DAILY: 0일 ~ 30일 사이 그룹핑<br> - BY_DAY_OF_WEEK: 월화수목금토일 그룹핑<br> 예: statsType을 BY_DAY_OF_WEEK로 설정하면, 30일치를 조회하더라도 요일(월~일) 기준으로 데이터가 그룹화됩니다.  |
...
| stats.rows | Array | O | EVENT_DATE_TIME 필드을 제외한 나머지 필드은 이벤트 카테고리에 따라 응답됩니다.<br><br> |
