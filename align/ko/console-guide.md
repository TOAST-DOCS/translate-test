...
| 항목 | 설명 |
| --- | --- |
| 이름 | 생성할 볼륨의 이름입니다. 볼륨 이름으로 NFS 또는 CIFS의 접근 경로를 만듭니다. 이름은 100자 이내의 영문자와 숫자, 일부 기호('-', '_')만 입력할 수 있습니다. |
| 설명 | 볼륨의 설명입니다. |
| 크기 | 생성할 볼륨의 크기입니다. 최소 300GB부터 최대 10,000GB까지 입력할 수 있습니다. |
| 프로토콜 | 볼륨에 접근할 프로토콜을 선택합니다. CIFS와 NFS를 지원하며 동시에 사용할 수 없습니다. |
...
| 스냅숏 자동 생성 | 매일 1회 지정한 시간에 자동으로 생성합니다. 최대 저장 개수에 도달하면 자동으로 생성된 스냅숏 중 가장 먼저 만들어진 스냅숏이 삭제됩니다.  |
...
> [참고]
> 프로젝트별로 사용 가능한 서브넷은 총 3개로 제한됩니다. 서브넷 개수 한도를 높이려면 고객지원에 문의하세요.

<a id="create_volume.cifs"></a>
#### CIFS 인증 정보 관리
CIFS 프로토콜을 사용하려면 CIFS 인증 정보를 생성해야 합니다. 인증 정보는 프로젝트 단위로 관리되며, CIFS 볼륨마다 접근할 CIFS 인증 정보를 등록해야 합니다.
...
> [주의]
> 복제 대상 볼륨은 복제를 중지해야 크기를 변경할 수 있습니다.
>
...
> [주의]
> 볼륨을 삭제하면 스냅숏을 포함한 모든 데이터가 삭제됩니다. 삭제 후에는 데이터를 복구할 수 없습니다.
...
| 항목 | 설명 |
| --- | --- |
| 이름 | 스냅숏의 이름을 표시합니다. 시스템이 생성한 경우 지정된 규칙에 따라 이름이 정해집니다. |
...
기본 조회 기간은 마지막 1시간이며, 조회 기간 필터에서 원하는 기간을 조회할 수 있습니다. 모니터링 지표 설명은 아래 표에서 확인할 수 있습니다.
...
대상 프로젝트별 선택 가능한 리전 범위는 아래 표에서 확인할 수 있습니다.
...
> [참고]
> 복제 설정으로 생성된 대상 볼륨은 NAS 서비스 요금 정책에 따라 볼륨 용량 요금이 청구됩니다.
>
...
| 항목 | 설명 |
| --- | --- |
| &lt;nas source&gt; | 볼륨 정보<br/>예: 192.168.0.241:/data |
