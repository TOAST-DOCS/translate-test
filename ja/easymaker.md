...
機械学習開発に必要な基本パッケージがインストールされているJupyter(ジュピター)ノートブックを作成・管理します。
...
学習後TensorBoard画面で結果指標を確認するには、学習スクリプト作成時にTensorBoardログ保存空間を指定位置(`EM_TENSORBOARD_LOG_DIR`)に設定する必要があります。
