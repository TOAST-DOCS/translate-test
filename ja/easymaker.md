...
機械学習開発のための必須パッケージがインストールされているJupyterノートブックを作成して管理します。

### ノートブック一覧

ノートブックの状態が表示されます。主な状態は下の表を参考にしてください。
...
学習後にTensorBoard画面で結果指標を確認するために、学習スクリプト作成時にTensorBoardログ保存場所を指定された位置(`EM_TENSORBOARD_LOG_DIR`)に設定する必要があります。
...
![ハイパーパラメータ入力画面](https://static.toastoven.net/prod_translate/ja/console-guide_appendix_hyperparameter_ko.png)