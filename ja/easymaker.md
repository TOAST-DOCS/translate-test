...
| ACTIVE (UNHEALTHY) | ノートブックアプリケーションが正常に稼働していない状態です。ノートブックを再起動してもこのステータスが続く場合はカスタマーサポートにお問い合わせください。 |
| STOPPED            | ノートブックを停止した状態です。                                                                                             |

### TensorBoard活用のための指標ログ保存

学習後TensorBoard画面で結果指標を確認するために、学習スクリプト作成時にTensorBoardログ保存スペースを指定された位置(`EM_TENSORBOARD_LOG_DIR`)に設定する必要があります。
