...
機械学習開発に必要なパッケージがインストールされたJupyter(ジュピター)ノートブックを作成・管理します。
...
学習後TensorBoard画面で結果指標を確認するために、学習スクリプト作成時にTensorBoardログ保存領域を指定の位置(`EM_TENSORBOARD_LOG_DIR`)に設定する必要があります。
