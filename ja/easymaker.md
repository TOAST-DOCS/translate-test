...
ノートブックの状態が表示されます。主な状態は、以下の表を参照してください。
...
| ACTIVE (UNHEALTHY) | ノートブックアプリケーションが正常に稼働していない状態です。ノートブックを再起動した後もこの状態が継続する場合は、カスタマーサポートにお問い合わせください。 |
| STOPPED            | ノートブックを停止した状態です。                                                                                                       |

### TensorBoardの活用のためのメトリクスログ保存

学習後にTensorBoard画面で結果メトリクスを確認するために、学習スクリプト作成時にTensorBoardログ保存スペースを指定された場所（`EM_TENSORBOARD_LOG_DIR`）に設定する必要があります。
