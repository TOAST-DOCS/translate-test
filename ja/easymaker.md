...
機械学習開発に必要なパッケージがインストールされているJupyter(ジュピター)ノートブックを作成および管理します。

### ノートブック一覧

ノートブックのステータスが表示されます。主なステータスは以下の表をご参照ください。
...
学習後TensorBoard画面で結果指標を確認するため、学習スクリプト作成時にTensorBoardログ保存領域を指定されたロケーション(`EM_TENSORBOARD_LOG_DIR`)に設定する必要があります。
