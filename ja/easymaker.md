...
ノートブックの状態が表示されます。主な状態については、以下の表をご参照ください。
...
### TensorBoardを利用するためのメトリクスログ保存

学習後TensorBoardの画面で結果メトリクスを確認するには、学習スクリプト作成時にTensorBoardログ保存スペースを指定された場所(`EM_TENSORBOARD_LOG_DIR`)に設定する必要があります。
