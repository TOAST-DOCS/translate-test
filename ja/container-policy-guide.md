...
!!! tip "ヒント"
    2026年5月時点では、**ライフサイクル**設定のみサポートしており、今後より多くの機能に拡大適用される予定です。
{% endif %}
...
| policy | Query | String | Y | ポリシー照会のためのクエリパラメータ{% if release_2026_08 %}<br>値を指定しない場合は全体のポリシードキュメントを、機能別の最上位キーを指定した場合は該当機能のポリシーのみ照会します。{% else %} (値なしで使用){% endif %} |
...
{% else %}
ポリシーはリクエストに含まれる最上位キー（例: lifecycle）単位で上書きされ、含まれないキーの設定は維持されます。
{% endif %}
...
| `rules` | Array | N | 条件ルールリスト | {% if release_2026_08 %}最大30個{% else %}省略するか、空の配列(`[]`)に設定すると条件ルールがすべて削除されます。{% endif %} |
