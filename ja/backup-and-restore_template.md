...
バックアップ後、バックアップファイルをObject Storageにエクスポートできます。増分バックアップはサポートされません。
...
内部バックアップストレージに保存されたバックアップファイルをObject Storageにエクスポートできます。増分バックアップはサポートされません。
...
(2) バックアップログファイルの最終行に `completed OK!` があるかどうかを確認します。`completed OK!` がない場合は、バックアップが正常に終了していないため、ログファイルに記録されているエラーメッセージを参照して、バックアップを再度実行します。
...
(2) Object Storage のバックアップを復元したいサーバーにダウンロードします。
