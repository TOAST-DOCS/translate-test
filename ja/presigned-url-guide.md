...
S3互換APIを使用するには、まずAWS EC2形式のS3 API認証情報（Access Key ID + Secret Access Key）を発行する必要があります。認証情報は、コンソールまたはAPIを使用して発行できます。コンソールを使用した認証情報の発行については「[S3 API認証情報](console-guide/#s3-api-credentials)」を、その他の詳細についてはAmazon S3互換APIガイドの「[S3 API認証情報](s3-api-guide/#s3-api-credential)」を参照してください。
