...
## DBインスタンスグループの詳細 { #db-instance-group-details }

DBインスタンスの一覧を**グループ**画面で表示した後、DBインスタンスグループを選択すると、グループの詳細情報を確認できます。グループ詳細画面には次のタブが表示されます。

| タブ | 説明 |
|---|---|
| 基本情報 | DBインスタンスグループの名前とID、高可用性構成、PrimaryおよびStandbyの名前、Ping設定を確認します。 |
| DBスキーマ & ユーザー | グループに属するDBインスタンスのDBスキーマとユーザーを管理します。DBスキーマとユーザー機能は、個別のDBインスタンスの詳細ではなく、DBインスタンスグループの詳細で提供します。 |
...
| 項目 | 説明 |
|--------------------|------------------------------------------------------------------------|
| DBインスタンスグループ名 | 1〜100 文字の英字、数字、`-`、`_`、`.` を使用できます。先頭文字は英字である必要があります。 |
| Primary名 | グループ名とは別に管理されます。 |
| 高可用性の有無 | シングル構成は高可用性構成に切り替えることができ、高可用性構成はシングル構成に切り替えることができます。 |
| Standby名 | 高可用性を新たに設定する際に入力します。Primary名と同じにすることはできません。名前の規則は Primaryと同じです。 |
| Ping間隔 | 高可用性構成で 1〜600 秒の範囲で設定します。 |
| Ping方式 | 高可用性構成で `INSERT` または `SELECT` を選択します。 |
| DBスキーマ & ユーザー直接制御 | グループに属する DBインスタンスのスキーマとユーザー直接制御の使用有無を変更します。 |

!!! danger "注意"
    * 高可用性を無効にすると、Standbyが削除されシングル構成に切り替わります。Standbyにのみ存在するデータや設定がないことを確認してから進めてください。
    * フェイルオーバーが進行中の DBインスタンスグループでは、高可用性の有無を変更することはできません。
    * プライベートネットワークでは Primary名を変更することはできません。また、既存の高可用性構成の Standby名は変更できません。高可用性を新たに設定する際にのみ Standby名を入力できます。
    * 高可用性構成では、Primaryと Standbyの名前は互いに異なる必要があります。
