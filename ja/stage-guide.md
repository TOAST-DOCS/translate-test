...
## Dev Tools > Pipeline > ステージガイド { #dev-tools-pipeline-stage-guide }
...
ユーザーが直接構成したHelm package fileまたは[チャートリポジトリ](./environment-config/#chart-repository)を使ってビルドできます。
...
- ソースビルド設定
    - **環境設定**の**イメージストア設定**で追加した[イメージストア](./environment-config/#image-registry)を選択できます。
...
- **リソースバージョニングを使用**を設定できます。パイプラインサービスのデフォルト動作であり、有効化することをお勧めします。詳細については、以下の**リソースバージョニング**を参照してください。
...
Pipelineサービスは ConfigMap、Secret リソースを配布する際、デフォルトで名前の末尾にバージョン (-v000、-v001、…) を付けた新しいリソースを作成し、
同じ配布に含まれるワークロードが該当リソースを参照する部分 (`volume`、`env`、`envFrom` など) をバージョン付きの名前に自動的に変更するリソースバージョン管理機能を提供します。
この機能により、設定変更の履歴がバージョンごとに保持され、ロールバック時にワークロードとともに以前の設定に戻すことができます。

**배포 - Deploy** ステージで**リソースバージョン管理を使用**を無効にすると、そのステージが配布するリソースはマニフェストに定義された元の名前のまま配布され、Pipelineサービスのリソースバージョン管理機能を使用できません。
オペレーター、コントローラーなど配布マニフェスト外でリソースを元の名前で直接参照する場合にのみ、無効にすることをお勧めします。
...
リソースバージョン管理オプションは、そのステージが配布するすべてのリソースに適用されます。
特定のリソースのみ異なる設定にするには、マニフェストの `metadata.annotations` に `strategy.spinnaker.io/versioned` アノテーションを `"true"` または `"false"` の値で追加することで、リソース単位で設定できます。
リソースバージョン管理の適用有無は、次の優先順位で決定されます。

1. リソースの `strategy.spinnaker.io/versioned` アノテーション
2. Deployステージの**リソースバージョン管理を使用**設定

**制約事項**
* バージョン管理を無効にしたリソースはバージョン履歴が残らないため、**배포 - Rollout undo** ステージおよび**配布対象管理**ワークロードのロールバック機能を使用できません。ロールバックを実行しても、以前の ConfigMap、Secret の設定は復元されません。
...
### 機能 - 承認管理 { #features---approval-management }
...
### 機能 - Judgement(実行管理) { #feature---judgement-run-management }
