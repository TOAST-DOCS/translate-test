...
NAS for BigData は、クラウド環境で大容量ファイルストレージを手軽に活用できる完全マネージド型の NAS（Network-Attached Storage）サービスです。標準 NFSプロトコル（Network File System）をベースに、クラウドインスタンスから簡単にマウントでき、ローカルディスクと同様にデータの読み書きができます。

$[ overview_capacity_prefix ]$拡張可能な大容量ストレージを提供し、インスタンス間のファイル共有、大規模データ分析、バックアップなど、さまざまな用途に柔軟に対応できます。
...
{% if overview_capacity_prefix %}最大 $[ max_size_text ]$までストレージ容量を拡張できます。$[ scale_description ]$ {% endif %}大規模なデータを扱うプロジェクトでも、物理機器の増設なしにコンソールからリアルタイムで容量を調整でき、運用負担を軽減します。ボリュームサイズの変更はデータ損失なしに反映され、このような拡張性と弾力性を基盤として、柔軟にデータを管理できます。
...
コンソールから複雑な設定を行わなくても、迅速にファイル水準のストレージを構成できます。また、Network ACL サービスで IP ベースのアクセス制御ポリシーを設定できるため、多数のインスタンスが接続された環境でも、セキュリティと柔軟性を同時に確保できます。
