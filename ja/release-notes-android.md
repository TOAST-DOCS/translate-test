...
* Googleの OOAP (Out-Of App Purchases) 購入が成功すると、Gamebase Event Handler の Purchase Updated イベントが発生します。
...
* ログイン後、またはアプリをバックグラウンドからフォアグラウンドに復帰した際に実行される自動再処理が成功すると、Gamebase Event Handler の Purchase Updated イベントが発生します。
    * [Game > Gamebase > Android SDK 사용 가이드 > ETC > Additional Features > Gamebase Event Handler > Purchase Updated](./aos-etc/#gamebase-event-handler-purchase-updated)
...
* AGP 8.0未満の環境でR8バージョンを上げていない場合、2.81.0適用時にゲームビルドが失敗していた現象を修正しました。
...
* 2.80.0バージョンで追加されたPendingイベント関連のロジックにより、IAPサーバーに負荷がかかる問題を修正
...
* targetSdk 36をサポートします。targetSdk 36ビルドをAndroid 16端末で実行した際、Webビューの「戻る」が正常に動作しない現象を修正しました。
* 内部ロジックの改善
...
* 米国テキサス、ユタ、ルイジアナのような管轄権の特定年齢確認法律による遵守義務を満たすのに役立つよう、Google Play Age Signalsベースの年齢確認APIが追加されました。
    ...
    * Play Age Signalsライブラリのバージョンがベータ(0.0.1-beta02)状態であるため、APIでは常に例外が発生します。
        * 今後正常に動作させるためには、Play Age Signalsライブラリ 0.0.2バージョンが適用されたGamebase Android SDK 2.78.0を使用してください。
...
* 外部SDKのアップデート: Hangame Android SDK(1.17.3)
* 内部ロジックの改善
...
* 外部SDKのアップデート: Facebook Android SDK(18.0.0)
* 内部ロジックの改善

<a id="731-2025-08-12-bug-fixes"></a>
#### 不具合の修正

* AGP 8.5でビルドした際に、NAVERログインが失敗する問題を修正しました。
* 利用規約→ 「もっと見る」をタップした際に、パンチホールのある端末でダイアログのサイズが画面を超える問題を修正しました。
...
* **Gamebase.requestLastLoggedInProvider(GamebaseDataCallback&lt;String&gt;)非同期API**を追加しました。
    * **Gamebase.getLastLoggedInProvider()同期API**がタイミング上、正常な値を返せない場合があります。
    * **gamebase-adapter-auth-gpgs-autologin**モジュールをビルドに含める場合、GPGSサーバーからデータを取得する時間が必要なので、Gamebase初期化直後にgetLastLoggedInProvider()同期APIを呼び出すと正常な値を取得できません。
...
* **GamebaseWebViewConfiguration.Builder.setCutoutAreaColor() API**を追加しました。
    * GamebaseWebViewの **GamebaseWebViewConfiguration.Builder.renderOutsideSafeArea() API**を**false**に設定した場合、 cutout領域に自動的にpadding余白を追加します。
...
* **gamebase-adapter-auth-gpgs-autologin** モジュールをビルドに含める場合、Gamebase初期化と同時に**Gamebase.getLastLoggedInProvider()同期API**を呼び出すと内部データが初期化されずnullが返されましたが、この場合、**'NOT\_INITIALIZED\_YET'**という文字列を返すように内部ロジックを変更しました。
* **GamebaseWebViewConfiguration.Builder.renderOutsideSafeArea() API**を**false**に設定した場合でもcutout領域までWebビューを全て表示するように(**edge-to-edge**)内部ロジックを変更しました。
