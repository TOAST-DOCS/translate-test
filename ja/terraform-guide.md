...
テラフォームはterraformバイナリファイルを起点にローカル環境や配布サーバーのようなリモート環境で目的のターゲットを呼び出す方式で実行されます。この時「目的のターゲット」は呼び出す方式が異なりますが、ターゲットのプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用をします。ここでテラフォームがターゲットとの相互作用ができるようにするのが「プロバイダー」です。
...
Terraformを利用したインフラの構築は通常、以下のようなライフサイクル（ライフサイクル）を持ちます。
...
次のセクションでは、これらのステップを例と共に詳しく説明します。


<a id="create-tf-files"></a>
### tfファイルの作成

プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめるか、リソース別に別々のtfファイルとして作成することもできます。Terraformは
