...
Terraformはterraformバイナリファイルを起点に、ローカル環境または配布サーバーなどのリモート環境で目的のターゲットを呼び出す方式で実行されます。この場合、「目的のターゲット」は呼び出し方式が異なりますが、ターゲットのプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここで、Terraformが目的のターゲットと相互作用できるようにするのが「プロバイダー」です。
...
まず、構築するインフラ形状をtfファイルに作成します。作成されたtfファイルに基づく構築計画は、以下のように`plan`コマンドで確認します。

```
$ terraform plan
```

構築計画に問題がなければ、`apply`コマンドを使用してリソースを生成、修正、削除します。
...
プロバイダー設定ファイルがあるパスでtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめるか、リソースごとに別のtfファイルで作成できます。Terraformは、作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。

以下は、`instance.tf`ファイルにインスタンスを生成するリソースを定義したtfファイルの例です。
