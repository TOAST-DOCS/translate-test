...
Terraformはterraformバイナリファイルを起点にローカル環境または配布サーバーのようなリモート環境から希望するターゲットを呼び出す方式で実行される。この時「希望するターゲット」は呼び出し方式は異なりますが、ターゲットの供給者、つまりプロバイダーが提供するAPIを呼び出して相互作用をする。ここでTerraformがターゲットとの相互作用ができるようにするのが「プロバイダー」です。
...
Terraformを利用したインフラ構築は通常、以下のようなライフサイクル(ライフサイクル)を持つです。
...
まず構築するインフラ形状をtfファイルに作成します。作成されたtfファイルに基づいた構築計画は以下のように`plan`コマンドで確認します。
...
次のセクションではこれらのステップを例題とともにより詳しく説明します。


<a id="create-tf-files"></a>
### tfファイル作成

プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルに集めたり、リソース別に別々のtfファイルで作成することも可能です。Terraformは作成された全体tfファイルを一度に読み込んで構築計画を立てます。
