...
NHN Cloudは、HashiCorpの公式パートナーとして、[Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest)を通じてTerraformプロバイダーを提供しています。

Terraformは、terraformバイナリファイルを起点として、ローカル環境または配布サーバーなどのリモート環境から目的のターゲットを呼び出すことで実行されます。この場合、「目的のターゲット」は呼び出し方法が異なりますが、ターゲットのプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここで、Terraformがターゲットとの相互作用ができるようにするものが「プロバイダー」です。
...
Terraformを使用したインフラ構築は、通常、以下のようなライフサイクルを持ちます。
...
まず、構築するインフラの形状をtfファイルに記述します。作成されたtfファイルに基づく構築計画は、以下のように`plan`コマンドで確認します。

```
$ terraform plan
```

構築計画に問題がない場合、`apply`コマンドを使用してリソースを作成、修正、削除します。

```
$ terraform apply
```

次のセクションでは、これらのステップをサンプルとともにより詳しく説明します。


<a id="create-tf-files"></a>
### tfファイル作成

プロバイダー設定ファイルがあるパスでtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめるか、リソースごとに別々のtfファイルとして作成することができます。Terraformは、作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。
