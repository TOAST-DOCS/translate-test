...
Terraformはterraformバイナリファイルを起点に、ローカル環境または配備サーバーのようなリモート環境から目的のターゲットを呼び出す方式で実行されます。この際、「目的のターゲット」は呼び出し方法が異なりますが、ターゲットのプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformがターゲットとの相互作用を可能にするのが「プロバイダー」です。
...
まず、構築するインフラ形状をtfファイルに記述します。作成されたtfファイルに基づく構築計画は、以下のように`plan`コマンドで確認します。
...
プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめるか、リソースごとに別々のtfファイルで作成することも可能です。Terraformは作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。
