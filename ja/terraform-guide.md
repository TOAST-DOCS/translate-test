...
Terraformは、terraformバイナリファイルを起点に、ローカル環境や、デプロイサーバーのようなリモート環境で目的のターゲットを呼び出す方式で実行されます。この時、「目的のターゲット」は呼び出し方式は異なりますが、ターゲットのプロバイダー、つまり、プロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformが目的のターゲットと相互作用できるようにするのが「プロバイダー」です。
...
Terraformを使用したインフラ構築は、通常以下のようなライフサイクル(生命周期)を持ちます。
...
まず、構築するインフラの形状をtfファイルに記述します。記述されたtfファイルに基づいた構築計画は、以下のように`plan`コマンドで確認します。
...
次のセクションでは、これらのステップについてサンプルと共に詳しく説明します。


<a id="create-tf-files"></a>
### tfファイルの作成

プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめるか、リソースごとに別々のtfファイルで作成することも可能です。Terraformは、記述されたすべてのtfファイルを一度に読み取って、構築計画を立案します。
