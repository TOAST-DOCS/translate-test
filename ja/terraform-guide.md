...
Terraformはterraformバイナリファイルを始めとしてローカル環境や導入サーバーのような遠隔環境で希望する対象を呼び出す方式で実行される。このとき「希望する対象」は呼び出す方式が異なりますが、対象の供給者、つまりプロバイダーが提供するAPIを呼び出して相互作用をします。ここでTerraformが対象との相互作用ができるようにするのが「プロバイダー」です。
...
まず構築するインフラ形状をtfファイルに作成します。作成されたtfファイルに従った構築計画は、以下のように`plan`コマンドで確認します。
...
プロバイダー設定ファイルがあるパスでtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめたり、リソース別に別々のtfファイルで作成することもできます。Terraformは
