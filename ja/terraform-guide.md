...
Terraformはtarraformバイナリファイルをスタートアップとして、ローカル環境またはデプロイサーバーなどのリモート環境で対象を呼び出す方式で実行される。この時「対象」は呼び出し方式が異なりますが、対象のプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformが対象と相互作用できるようにするのが「プロバイダー」です。
...
まず構築するインフラの形状をtfファイルに記述します。記述されたtfファイルに基づいた構築計画は以下のように`plan`コマンドで確認します。
...
プロバイダー設定ファイルのあるパスでtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめたり、リソース別に別々のtfファイルで作成することもできます。Terraformは記述されたすべてのtfファイルを一度に読み込んで構築計画を策定します。
