...
NHN Cloudは、HashiCorp社の公式パートナーとして、[Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest)を通じてTerraformプロバイダーを提供しています。

Terraformは、terraformバイナリファイルを起点に、ローカル環境または配置サーバーなどのリモート環境で目的の対象を呼び出す方式で実行されます。この場合、「目的の対象」は呼び出す方式が異なりますが、対象の供給者、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformが対象との相互作用を可能にするのが「プロバイダー」です。
...
まず、構築するインフラ構成をtfファイルに記述します。作成したtfファイルに基づいた構築計画は、以下のように`plan`コマンドで確認できます。
...
次のセクションでは、これらのステップをより詳しい例とともに説明します。


<a id="create-tf-files"></a>
### tfファイルの作成

プロバイダー設定ファイルがあるパスでtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめたり、リソースごとに個別のtfファイルとして作成することもできます。Terraformは、作成されたすべてのtfファイルを一度に読み取って、構築計画を立てます。
