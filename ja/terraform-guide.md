...
<a id="terraform"></a>
## Terraform
Terraformは、インフラストラクチャを簡単に構築し、安全に変更し、効率的にインフラストラクチャの構成を管理できるオープンソースツールです。Terraformの主な特徴は以下のとおりです。
...
* **以下の例で使用されたTerraformのバージョンは1.0.0です。**
...
## Terraformインストール
[Terraformダウンロードページ](https://www.terraform.io/downloads.html)でローカルPCのOSに対応するファイルをダウンロードします。ファイルを解凍して希望するパスに配置し、環境設定に該当パスを追加すればインストールが完了します。
...
Terraformはterraformバイナリファイルを起点として、ローカル環境やデプロイサーバーなどのリモート環境で目的のターゲットを呼び出す方式で実行されます。この時「目的のターゲット」は呼び出し方式が異なりますが、ターゲットの提供者、つまりプロバイダーが提供するAPIを呼び出してインタラクションを行います。ここでTerraformがターゲットとのインタラクションを可能にするのが「プロバイダー」です。
...
* **auth_url**
    ...
    * NHN Cloudコンソールの**Compute > Instance > 管理**メニューで**APIエンドポイント設定**ボタンを押してアイデンティティサービス(identity) URLを確認します。
* **region**
...
まず、構築するインフラ構成をtfファイルに記述します。作成されたtfファイルに基づく構築計画は以下のように`plan`コマンドで確認します。
...
プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめたり、リソースごとに別々のtfファイルとして作成することも可能です。Terraformは作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。
