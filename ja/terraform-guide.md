...
* **Change Automation**
    * 複数の場所に同じ構成のインフラストラクチャを構築し、変更できるよう自動化できます。
...
* **バージョンを含むコンポーネントの名前と数字は変更される可能性がありますので、確認してからご利用ください。**


<a id="terraform-installation"></a>

## Terraformのインストール
[Terraformダウンロードページ](https://www.terraform.io/downloads.html)で、ローカルPCのオペレーティングシステムに適合するファイルをダウンロードします。ファイルを解凍して任意のパスに配置した後、環境設定でそのパスを追加すれば、インストールが完了します。
...
Terraformは、terraformバイナリファイルを起点として、ローカル環境やデプロイサーバーのようなリモート環境で対象を呼び出す方式で実行されます。この際、「対象」は呼び出し方式がそれぞれ異なりますが、対象の供給者、つまりプロバイダーが提供するAPIを呼び出して相互作用を行います。ここで、Terraformが対象との相互作用を可能にするのが「プロバイダー」です。
...
providerのバージョンは[NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest)の`VERSION`情報を参考にして作成します。
...
* **password**
    ...
    * APIパスワードの設定方法は**ユーザーガイド > Compute > Instance > API使用準備**を参照してください。
* **auth_url**
    * NHN Cloud アイデンティティサービスのアドレスを指定します。
...
構築計画に問題がなければ`apply`コマンドを利用してリソースを作成、変更、削除します。
...
プロバイダ設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめるか、リソースごとに別途のtfファイルとしても作成可能です。Terraformは作成された全体のtfファイルを一度に読み込んで構築計画を立案します。
