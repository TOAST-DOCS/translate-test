...
* **Resource Graph**
    ...
    * 依存関係グラフを作成し、このグラフに基づいて計画を立て、この計画を適用した時に変更されるインフラ状態を確認できます。
* **Change Automation**
...
## Terraform インストール
[Terraformダウンロードページ](https://www.terraform.io/downloads.html)からローカルPCのオペレーティングシステムに対応するファイルをダウンロードします。ファイルの圧縮を解除し、目的のパスに配置してから、環境設定にそのパスを追加するとインストールが完了します。
...
* **auth_url**
    * NHN Cloud IDプロバイダーサービスアドレスを指定します。
    * NHN Cloudコンソールの**Compute > Instance > 管理**メニューから**APIエンドポイント設定**ボタンをクリックしてIDプロバイダー(identity) URLを確認します。
* **region**
