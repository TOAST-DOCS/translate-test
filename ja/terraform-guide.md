...
Terraformは、Terraform バイナリファイルを起点としてローカル環境または配布サーバーなどの遠隔環境で、望まれた対象を呼び出す方式で実行されます。この時、「望まれた対象」は呼び出す方式が異なりますが、対象の供給者、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformが対象との相互作用ができるようにするのが「プロバイダー」です。
...
まず構築するインフラ形状をtfファイルに記述します。記述されたtfファイルに基づく構築計画は、以下のように`plan`コマンドで確認します。
...
プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめることも、リソースごとに別のtfファイルとして作成することも可能です。Terraformは、記述されたすべてのtfファイルを一度に読み込んで構築計画を立案します。
