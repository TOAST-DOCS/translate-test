...
Terraformは、Terraformバイナリファイルから始まり、ローカル環境または配布サーバーなどのリモート環境で目的の対象を呼び出す方式で実行されます。このとき「目的の対象」は呼び出し方式が異なりますが、対象のプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformが対象と相互作用できるようにするのが「プロバイダー」です。
...
Terraformを使用してインフラを構築する場合、通常は以下のようなライフサイクルを経ます。
...
まず、構築するインフラの形状をtfファイルに記述します。記述されたtfファイルに基づいて構築計画を確認するには、以下のように`plan`コマンドを使用します。
...
次のセクションでは、これらのステップについて例を用いてより詳しく説明します。


<a id="create-tf-files"></a>
### tfファイルの作成

プロバイダー設定ファイルがあるパスでtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめることも、リソースごとに別々のtfファイルを作成することも可能です。Terraformは、作成されたすべてのtfファイルを一度に読み込んで構築計画を立案します。
