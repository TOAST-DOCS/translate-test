...
<a id="terraform"></a>
## Terraform
Terraformは、インフラストラクチャを簡単に構築し、安全に変更し、効率的にインフラストラクチャの形状を管理できるオープンソースツールです。Terraformの主な特徴は以下のとおりです。

* **Infrastructure as Code**
    * インフラストラクチャをコードとして定義することで、生産性と透明性を向上させることができます。
    * 定義したコードを簡単に共有でき、効率的に協業できます。
* **Execution Plan**
    * 変更計画と変更適用を分離することで、変更内容を適用する際に発生する可能性のあるミスを削減できます。
* **Change Automation**
    * 複数の場所に同じ構成のインフラストラクチャを構築し変更できるよう自動化できます。
    * インフラストラクチャを構築するのにかかる時間を節約でき、ミスも削減できます。
...
* **バージョンを含む構成要素の名前と数字は変更される可能性がありますので、確認後ご利用ください。**


<a id="terraform-installation"></a>

## Terraform インストール
[Terraform ダウンロードページ](https://www.terraform.io/downloads.html)から、ローカルPCのオペレーティングシステムに適したファイルをダウンロードします。ファイルを解凍して任意のパスに配置し、環境設定にそのパスを追加すればインストールが完了します。
...
NHN Cloudは、HashiCorp社の公式パートナーとして[Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest)を通じてTerraform providerを提供しています。

Terraformは、terraform バイナリファイルを起点として、ローカル環境や配布サーバーのような遠隔環境で目的の対象を呼び出す方式で実行されます。この時、「目的の対象」は呼び出し方式が異なりますが、対象の供給者、すなわちプロバイダーが提供するAPIを呼び出して相互作用を行います。ここでTerraformが対象との相互作用を可能にするものが「プロバイダー」です。
...
まず構築するインフラの構成をtfファイルに記述します。作成されたtfファイルに基づく構築計画は、以下のように`plan`コマンドで確認します。
...
プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめたり、リソースごとに別々のtfファイルで作成することも可能です。Terraformは作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。
