...
## サードパーティツール使用ガイド > Terraform使用ガイド
...
* **Execution Plan**
    * 変更計画と変更適用を分離し、変更内容を適用する時に発生する可能性がある実行ミスを減らすことができます。
* **Resource Graph**
    * 些細な変更がインフラ全体にどのような影響を与えるかを事前に確認できます。
    * 依存性グラフを作成してこのグラフを基に計画を立て、この計画を適用した時に変更されるインフラ状態を確認できます。
* **Change Automation**
    * 複数の場所に同じ構成のインフラを構築して変更できるよう自動化できます。
...
* **バージョンを含むコンポーネントの名前と数字は変更される可能性があるため、確認後に使用してください。**


<a id="terraform-installation"></a>
## Terraformのインストール
[Terraformダウンロードページ](https://www.terraform.io/downloads.html)でローカルPCのOSに合うファイルをダウンロードします。ファイルの圧縮を解除し、希望するパスに入れた後、環境設定に該当パスを追加すればインストールが完了します。
...
* **password**
    ...
    * APIパスワード設定方法は**ユーザーガイド > Compute > Instance > API使用準備**をご参照ください。
* **auth_url**
    * NHN Cloud ID サービスアドレスを明示します。
