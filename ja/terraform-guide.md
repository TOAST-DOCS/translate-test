...
Terraformはterraformバイナリファイルを始まりとしてローカル環境や配布サーバーのようなリモート環境で目的の対象を呼び出す方式で実行されます。この時「目的の対象」は呼び出す方式が互いに異なりますが対象の供給者、すなわちプロバイダーが提供するAPIを呼び出して相互作用をします。ここでTerraformが対象との相互作用をできるようにするのが「プロバイダー」です。
...
まず構築するインフラ形状をtfファイルに作成します。作成されたtfファイルに基づく構築計画は以下のように`plan`コマンドで確認します。
...
次のセクションではこれらのステップをサンプルと共に詳しく説明します。


<a id="create-tf-files"></a>
### tfファイル作成

プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめたり、リソース別に別のtfファイルとしても作成できます。Terraformは作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。
