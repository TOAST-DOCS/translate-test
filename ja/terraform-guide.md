...
このドキュメントはTerraformでNHN Cloudを使用する方法について説明します。
...
* **Resource Graph**
    ...
    * 依存関係グラフを作成し、このグラフに基づいて計画を立てて、この計画を適用した際に変更されるインフラの状態を確認できます。
* **Change Automation**
    * 複数の場所に同じ構成のインフラを構築し、変更できるように自動化できます。
    * インフラを構築するにかかる時間を節約でき、誤りも減らすことができます。
...
## Terraform のインストール
[Terraform ダウンロードページ](https://www.terraform.io/downloads.html)からローカルPCのオペレーティングシステムに対応するファイルをダウンロードします。ファイルを解凍して希望するパスに配置し、環境変数にそのパスを追加すればインストールが完了します。

以下はLinux(Ubuntu/Debian)でのインストール例です。
...
* **auth_url**
    * NHN Cloud 識別サービスのアドレスを指定します。
    * NHN Cloudコンソールの**Compute > Instance > 管理**メニューで**APIエンドポイント設定**ボタンをクリックして識別サービス(identity)のURLを確認します。
* **region**
