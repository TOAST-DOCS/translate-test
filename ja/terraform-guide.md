...
<a id="terraform"></a>
## Terraform
Terraformは、インフラを簡単に構築し、安全に変更し、効率的にインフラの形状を管理できるオープンソースツールです。Terraformの主な特徴は次の通りです。

* **Infrastructure as Code**
    * インフラをコードとして定義して生産性と透明性を向上させることができます。
    * 定義したコードを簡単に共有でき、効率的にコラボレーションできます。
* **Execution Plan**
    * 変更計画と変更適用を分離して、変更内容を適用する際に発生する可能性のある間違いを減らすことができます。
* **Resource Graph**
    * わずかな変更がインフラ全体にどのような影響を与えるかを事前に確認できます。
    * 依存性グラフを作成してこのグラフに基づいて計画を立て、この計画を適用した際に変更されるインフラの状態を確認できます。
* **Change Automation**
    * 複数の場所に同じ構成のインフラを構築し変更できるよう自動化できます。
...
## Terraformインストール
[Terraformダウンロードページ](https://www.terraform.io/downloads.html)でローカルPCのOSに合ったファイルをダウンロードします。ファイルを解凍して希望のパスに置き、環境設定に該当パスを追加するとインストールが完了します。
...
* **password**
    ...
    * APIパスワード設定方法は**ユーザーガイド > Compute > Instance > API使用準備**を参考してください。
* **auth_url**
    * NHN Cloud Identity サービスアドレスを明示します。
    * NHN Cloudコンソールの**Compute > Instance > 管理**メニューで**APIエンドポイント設定**ボタンをクリックしてIdentityサービス(identity) URLを確認します。
* **region**
