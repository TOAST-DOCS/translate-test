...
Terraformはterraformバイナリファイルを起点に、ローカル環境または配布サーバーなどのリモート環境で希望するターゲットを呼び出す方式で実行されます。このときの「希望するターゲット」は呼び出し方法が異なりますが、ターゲットのプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformがターゲットと相互作用できるようにするのが「プロバイダー」です。
...
まず構築するインフラの構成をtfファイルに作成します。作成したtfファイルに基づいた構築計画は、以下のように`plan`コマンドで確認します。
...
プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルにまとめるか、リソースごとに別のtfファイルで作成することも可能です。Terraformは作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。
