...
この文書は、TerraformでNHN Cloudを使用する方法について説明します。
...
* **Infrastructure as Code**
    * インフラをコードで定義して生産性と透明性を向上させることができます。
    * 定義したコードを簡単に共有でき、効率的に協業できます。
* **Execution Plan**
    * 変更計画と変更適用を分離して、変更内容を適用する時に発生する可能性があるミスを削減できます。
* **Resource Graph**
    * わずかな変更がインフラ全体にどのような影響を与えるか事前に確認できます。
    * 依存関係グラフを作成してこのグラフに基づいて計画を立て、この計画を適用した時に変更されるインフラ状態を確認できます。
* **Change Automation**
    * 複数の場所に同じ構成のインフラを構築し、変更できるように自動化できます。
    * インフラを構築するのにかかる時間を節約でき、ミスも削減できます。
...
* **バージョンを含む構成要素の名前と数字は変更される場合があるため、確認の上ご使用ください。**


<a id="terraform-installation"></a>
## Terraform インストール
[Terraform ダウンロードページ](https://www.terraform.io/downloads.html)でローカルPCのOSに合ったファイルをダウンロードします。ファイルを解凍して任意のパスに配置した後、環境設定に該当パスを追加するとインストールが完了します。
...
## Terraform 初期化
Terraformを使用する前に、次のようにプロバイダー設定ファイルを生成します。
...
* **password**
    ...
    * API パスワード設定方法は**ユーザーガイド > Compute > Instance > API 使用準備**を参考してください。
* **auth_url**
    * NHN Cloud アイデンティティサービスアドレスを明示します。
    * NHN Cloudコンソールの**Compute > Instance > 管理**メニューで**API エンドポイント設定**ボタンをクリックしてアイデンティティサービス(identity) URLを確認します。
* **region**
