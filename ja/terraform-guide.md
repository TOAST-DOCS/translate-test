...
このドキュメントは、TerraformでNHN Cloudを使用する方法について説明しています。
...
* **下記の例で使用しているTerraformバージョンは1.0.0です。**
* **バージョンを含むコンポーネントの名前と数字は変更される可能性があるため、確認してからご使用ください。**


<a id="terraform-installation"></a>
## Terraform のインストール
[Terraform ダウンロードページ](https://www.terraform.io/downloads.html)から、ローカルPCのオペレーティングシステムに対応するファイルをダウンロードします。ファイルを解凍して目的のパスに配置し、環境設定にそのパスを追加するとインストールが完了します。
...
provider バージョンは[NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest)の`VERSION`情報を参考にして記述します。
...
* **user_name**
    * NHN Cloud IDを使用します。
* **tenant_id**
    * NHN CloudコンソールのCompute > Instance > 管理メニューで**APIエンドポイント設定**ボタンをクリックしテナントIDを確認します。
* **password**
...
* **auth_url**
    ...
    * NHN CloudコンソールのCompute > Instance > 管理メニューで**APIエンドポイント設定**ボタンをクリックし、認証サービス(identity) URLを確認します。
* **region**
