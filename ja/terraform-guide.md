...
Terraformはterraformバイナリファイルを起点として、ローカル環境や構築サーバーなどのリモート環境から目的のターゲットを呼び出す方式で実行されます。この時、「目的のターゲット」は呼び出し方式が異なることがありますが、ターゲットのプロバイダー、つまりプロバイダーが提供するAPIを呼び出して相互作用します。ここでTerraformがターゲットと相互作用できるようにするのが「プロバイダー」です。
...
Terraformを使用したインフラ構築は、通常以下のようなライフサイクル(生存期間)を持ちます。
...
次のセクションでは、これらのステップについてより詳しく例とともに説明します。


<a id="create-tf-files"></a>
### tfファイルの作成

プロバイダー設定ファイルがあるパスにtfファイルを作成します。複数のリソース設定を1つのtfファイルに集めたり、リソースごとに別々のtfファイルとして作成することもできます。Terraformは作成されたすべてのtfファイルを一度に読み込んで構築計画を立てます。
