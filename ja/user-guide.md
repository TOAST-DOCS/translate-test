...
| コンテナタイプ | コンテナのタイプ<br><ul><li>一般: 実行状態を維持する必要があるコンテナ</li><li>初期化: 一般コンテナ実行前に完了する必要があるコンテナ</li></ul>|
...
| シークレット | Secure Key Managerに保存した機密データファイルをコンテナディレクトリにマウントして使用することができます。 <ul><li>キーID：Secret Key Managerサービスの機密データタイプのIDを入力します。</li><li>コンテナマウントパス：コンテナのマウントパスを入力します。<ul><li>入力されたパスにファイルがマウントされます。</li></ul></li></ul> |
...
| テンプレートバージョン名 | テンプレートバージョン名。16文字以内の英小文字、数字、一部記号(`-`)のみ入力できます。| 
...
ワークロードを再起動すると、ロードバランサーの IP が変更されます。IP を変更しない場合は、API でロードバランサーの IP を指定してください。
