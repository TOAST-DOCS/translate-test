...
3. 詳細設定でシリーズ識別ラベルとグループラベルを指定します。
    - スキーマは静的であるため、直接入力しません。
    - **[例で見る]** で入力したラベルによってシリーズとグループがいくつに分かれるか確認できます。
4. **[追加]** ボタンをクリックし、完了ウィンドウに「準備が完了しました。」が表示されるまで待ちます。
    - 完了ウィンドウには収集方法（エンドポイント、リクエストヘッダ、リクエストボディの例、ルール）が併せて表示されます。
    - 一覧では、ステータスが `COMPLETED` と表示されます。
...
データソース作成完了画面、または詳細表示の**[収集方法]**タブで、エンドポイントとリクエストヘッダ、リクエストボディの例を**[コピー]**ボタンでコピーし、メトリクスを転送します。リクエストヘッダの認証トークンの箇所には、発行したトークンを入力します。
...
1. **[基本設定]** にアプリ名と説明を入力し、アプリタイプとして **[単変量時系列異常検出]** を選択します。

    ![アプリ作成 - 基本設定](../static/images/quick-start/이상탐지앱생성1.png){ height="70%" }

2. 詳細設定で、先ほど作成した指標データソースを選択します。
    - モデルリソースと再学習周期、検出オプション、結果転送を指定します。
    - 再学習周期を指定しない場合、アプリの作成時に一度だけ学習します。この場合、データがないデータソースではアプリを作成できないため、先にメトリクスを送信します。
...
各項目の詳細については、[コンソールユーザーガイド](./console-user-guide/#app-create-detail-univariate)の「単変量時系列異常検知の詳細設定」を参照してください。
...
![単変量時系列異常検出アプリ情報](../static/images/quick-start/이상탐지앱정보.png){ height="70%" }

2. **[グループ一覧]** タブでグループのステータスを確認します。
    - グループはメトリクスが届いてから登録されるため、アプリを作成した直後は一覧が空になっています。
    - 有効化待機中は判定に使用するデータを収集中であり、有効化されると検出結果が転送されます。
    - 検出開始時刻カラムで、グループがいつから結果を送信し始めたかを確認できます。
    - 推論状態カラムで、推論が正常に動作しているかを確認します。
...
各項目の詳細については、[コンソールユーザーガイド](./console-user-guide/#app-detail-univariate)の「単変量時系列異常検出アプリ詳細」を参照してください。