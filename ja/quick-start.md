...
3. 詳細設定でシリーズ識別ラベルとグループラベルを指定します。
    - スキーマは静的のため、直接入力しません。
    - **[サンプルを見る]** で入力したラベルによってシリーズとグループがいくつに分かれるか確認できます。

4. **[追加]** ボタンをクリックし、完了ウィンドウに「準備が完了しました。」と表示されるまで待ちます。
    - 完了ウィンドウには、収集方法（エンドポイント、リクエストヘッダ、リクエスト本文の例、ルール）が一緒に表示されます。
...
データソース作成完了ウィンドウまたは詳細表示の **[収集方法]** タブで、エンドポイント、リクエストヘッダ、リクエスト本文の例を **[コピー]** ボタンでコピーして指標を転送します。リクエストヘッダの認証トークンの箇所には、発行したトークンを入力します。
...
!!! tip "知っておくべきこと"
    アプリを作成した後は、同じ時系列の指標を1分に1つずつ途切れなく送信します。それより長い間隔で送信すると空白区間が生じ、精密モードで準備が完了しない場合があります。また、1分以内に複数の値を送信した場合は、先に到着した値のみが使用されます。より短い周期で収集する場合は、1分間の平均にまとめて送信します。
...
2. 詳細設定で、先ほど作成した指標データソースを選択します。
    ...
    - 再学習周期を指定しない場合は、アプリ作成時に一度だけ学習します。この場合、データのないデータソースではアプリを作成できないため、先に指標を送信します。

    ![アプリ作成 - 詳細設定](../static/images/quick-start/이상탐지앱생성2.png){ height="70%" }

3. 最終確認で入力内容を確認し、**[保存]** ボタンを押します。

    - 完了ウィンドウには、学習・デプロイの進行と結果が表示されるまでの時間が案内されます。その間も指標を送り続けます。
...
2. **[グループ一覧]** タブでグループの状態を確認します。
    ...
    - 有効化時刻カラムで、グループがいつから結果を送信し始めたかを確認できます。
