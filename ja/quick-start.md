...
3. 接続設定でデータソースタイプが**[ファイルアップロード]**であることを確認します。
4. 詳細設定で CSV ファイルを選択します。ファイルの先頭行がカラム名の場合は、**[最初の行はヘッダです]**をチェックします。主キーフィールド（例: `user_id`）を入力します。

5. **[タイプ推論]**ボタンをクリックすると、CSV サンプルからスキーマが自動的に入力されます。誤って推論されたタイプは手動で修正します。

    ![データソースの作成 - ファイル選択とタイプ推論](../static/images/quick-start/데이터소스생성모달2.png){ height="70%" }

6. **[追加]** ボタンをクリックすると、データソースが作成されます。
7. 同様の手順で、**ユーザー**、**アイテム**、**履歴**のデータソースをそれぞれ作成します。
