...
通知管理機能は、NHN Cloudで送信する通知ごとに、お客様自身が受信する対象者と通知方法(Email、SMS、Webフック)を設定できる機能です。
...
2. 通知リストで受信対象者を変更する通知を探し、**受信対象修正 > 修正** ボタンをクリックします。
    ...
    - 複数の通知の受信対象者を一度に変更するには、通知のチェックボックスを選択した後、通知リスト上部の**受信対象一括設定**ボタンを押します。

3. メンバー、通知受信グループ、ロール別に**通知受信対象および通知方法（Email、SMS）**を選択します。
    - Webフックは、Webフックをサポートする通知に限り、同じページの**Webhook設定**エリアで追加できます。
    - 通知ごとにサポートする通知方法が異なります。
    - 受信対象として通知受信グループを追加する場合、そのグループに設定された通知方法（Email、SMS）と各通知でサポートされている通知方法が一致している必要があります。一致している場合のみ、その方法で通知を受信できます。
...
6. **Webhook設定**
    ...
    - タイプ別に基本WebhookとカスタムWebhookを提供します。  (それぞれ最大5個までサポート)
...
- カスタムWebフック
    - カスタムWebフックは、対応するサービス通知の際にのみ送信できます。
    - Webhook名
    ...
    * イベント
        * 選択したサービスが提供するイベントの中から、Webフックで受信するイベントを選択できます。
    * 受信対象
  ...
  - リクエストデータ
      - リクエストデータには、選択したサービスとイベントが提供するパラメータを入力できます。
          - Ctrlキー + スペースキーを押して、該当するサービスとイベントがサポートするパラメータを確認できます。
  ...
  - 名前、サービス、イベント、受信対象を入力した後、**確認**ボタンをクリックしてWebhookを作成します。
