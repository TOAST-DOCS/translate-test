...
{% if release_2026_05 %}ライフサイクルの設定方法の詳細については、[ライフサイクルルールの適用方法](container-policy-guide/#lifecycle-apply)を参照してください。{% endif %}
...
{%- endif %}
    ...
    <td>オブジェクトのライフサイクルを日単位で入力します。{% if not release_2026_05 %} 空白のままにすると、ライフサイクルの設定が解除されます。{% endif %}</td>
  </tr>
  <tr>
    <td>ライフサイクル満了動作</td>
...
{%- endif %}
{%- if ec %}
    Standardクラスのコンテナに保存されたオブジェクトを、ライフサイクルに従ってEconomyクラスのコンテナに移動することで、長期保管にかかるコストを削減できます。
...
ロールベースのアクセスポリシーに関係なく、設定した時間の間、指定したオブジェクトに自由にアクセスできる署名付き URLを生成します。{% if release_2026_08 %} 詳細な使用方法については、[署名付き URLガイド](presigned-url-guide/)を参照してください。{% endif %}
