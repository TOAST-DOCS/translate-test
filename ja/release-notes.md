<!-- machine_translated: true -->

<!-- pre-align:aligned sig=1ab82bf77f7a -->

<a id="ai-service-speech-to-text-release-notes"></a>
## AI Service > Speech to Text > リリースノート { #ai-service-speech-to-text-release-notes }

<a id="july-7-2026"></a>
### 2026. 07. 07. { #july-7-2026 }

* ドメインの変更
* 容量制限の変更
* 音声ファイルの時間制限の変更
* API v1.1、v2.1 リリース
  * User Access Key トークン認証を使用する API が追加されました。
  * User Access Key トークンの発行および使用の詳細については、[User Access Key トークン](/nhncloud/ja/public-api/user-access-key-token)を参照してください。

<a id="august-14-2025"></a>
### 2025. 08. 14. { #august-14-2025 }

<a id="august-14-2025-added-features"></a>
#### 機能追加

* API v2.0リリース
* 非同期APIリリース
* ユーザー単語リストパラメータ追加

<a id="may-13-2025"></a>
### 2025. 05. 13. { #may-13-2025 }

* 音声認識性能の改善

<a id="october-31-2023"></a>
### 2023. 10. 31. { #october-31-2023 }

* バグ修正
    * オーディオファイルに録音された音声がない場合、エラーとして結果を受け取れるように修正

<a id="february-28-2023"></a>
### 2023. 02. 28. { #february-28-2023 }

* コンソールのUI改善
    * 上部案内文言の削除
    * ツールチップの追加

<a id="december-27-2022"></a>
### 2022. 12. 27. { #december-27-2022 }

* 年齢別音声認識性能の改善

<a id="october-4-2022"></a>
### 2022. 10. 04. { #october-4-2022 }

* 後処理性能の改善
    * 数字、英字変換の改善
    * 文章の終止符の位置が改善

<a id="may-24-2022"></a>
### 2022. 05. 24. { #may-24-2022 }

* 音声認識性能の改善

<a id="february-22-2022"></a>
### 2022. 02. 22. { #february-22-2022 }

* Speech to Textサービスリリース
