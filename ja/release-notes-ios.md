...
* Log & Crash Search Symbol API v3 適用
    * v0.0.5 より User Access Token 認証が必要になります。
...
* Notification Hub メトリクス転送方式の改善
    * メトリクスの転送がリトライされても、同一のメトリクスが重複集計されないよう改善しました。

<a id="100-2026-09-15-nhn-cloud-push-bugfix"></a>
#### バグ修正

* Notification Hub トークンアップデート失敗の問題を修正
...
* PublicKey 取得失敗エラーの通知改善
    * Delegate登録前に発生した PublicKey 取得失敗エラーを、Delegate登録後に通知するよう改善しました。
...
* Notification Hub サポート
    * NHNCloudPush SDKで Notification Hub を利用できます。
    * NHNCloudPushConfiguration の serviceType プロパティに NHNCloudPushServiceTypeNotificationHub の値を設定して使用可能です。
...
* Instance Logger の NetworkInsight 安定性改善
