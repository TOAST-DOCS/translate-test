...
## Game > Gamebase > Android SDK ご利用ガイド > はじめる { #game-gamebase-android-developers-guide-getting-started }
...
|  | gamebase-adapter-auth-hangame | hangame-id-1.17.4 | Hangameログインをサポート | - |
...
* Android Studioビルドの場合
    * Firebaseプッシュを使用するには、以下のガイドに従ってFirebaseの設定を完了した後、google-services.jsonファイルをプロジェクトに含める必要があります。
        * [NHN Cloud > SDK使用ガイド > Push > Android > Firebase Cloud Messaging設定](/nhncloud-sdk/ja/push-android/#firebase-cloud-messaging-settings)
* Unityビルドの場合
...
| ------------- | ---------- | ----------- |
| com.nhncloud.sdk.push.deviceId.salt | String | 異なるアプリケーションが1つのGamebaseプロジェクトを共有する場合、プッシュが正常に動作しません。<br/>各アプリに対して異なる任意の 'salt' 値を指定する必要があります。 |
