...
3. テンプレートの使用有無を **[使用]** に選択してから、送信リクエストするテンプレートを選択します。
    * テンプレートは **Notification > KakaoTalk Bizmessage > ブランドメッセージ > テンプレート管理** タブで登録できます。
4. 代替送信メッセージの設定有無を選択します。
    * **[080受信拒否管理]** タブで代替送信の有無を **[使用]** に設定し、SMSサービスを連携する必要があります。連携後に代替送信が可能になります。
5. 080受信拒否番号の設定有無を選択します。
    * 080受信拒否番号の設定有無で **[設定しない（発信プロフィールの設定で送信）]** を選択した場合、発信プロフィールで設定した080受信拒否番号が使用されます。
    * **[共通内容で設定]** または **[ユーザー別設定]** を選択した場合、入力した080受信拒否番号が送信に使用されます。
    * **[080受信拒否管理]** タブでSMSサービスを利用する場合、SMSサービスに登録された080受信拒否番号のみ使用できます。
6. 受信者を追加します。
    * 受信者は携帯電話番号の形式で入力できます。
7. 受信者ごとにターゲティングタイプを選択します。
    * ブランドメッセージは、広告主のマーケティング受信同意（以下、マス動）ユーザーおよびチャンネル友達ユーザーを対象に送信できる広告性メッセージです。
    * ターゲティングタイプによって、指定した受信者のメッセージ受信可否が異なる場合があります。
        * M: 広告主マス動ユーザー（カカオトーク受信同意）
            * 広告主のマス動ユーザー（カカオトーク受信同意）を対象に広告性メッセージを送信します。
        * N: 広告主マス動ユーザー（カカオトーク受信同意）- チャンネル友達
            * 広告主のマス動ユーザー（カカオトーク受信同意）からチャンネル友達を除いて広告性メッセージを送信します。
        * I: 広告主送信リクエスト対象 ∩ チャンネル友達
            * 広告主の送信リクエスト対象のうち、チャンネル友達である場合にのみ広告性メッセージを送信します。
8. グループタグキーを入力します。（任意）
...
3. 成人向けメッセージかどうかを選択します。
    * 成人向けメッセージの場合、年齢認証が完了するまでチャットルームのメッセージにカバーが表示され、20歳以上のみ内容を確認できます。
4. メッセージタイプを選択します。
    * テキスト
        * 日本語・英語を問わず、スペースを含む 1,300 文字のテキスト＋リンクボタン最大 5 個（縦配列）
    * 画像
        * 日本語・英語を問わず、スペースを含む 1,300 文字のテキスト＋画像 1 枚＋リンクボタン最大 5 個（縦配列）
    * ワイド画像
        * 日本語・英語を問わず、スペースを含む 76 文字のテキスト＋画像 1 枚＋リンクボタン最大 2 個
    * ワイドアイテムリスト
        * 1 つのタイトルに 3〜4 個のリスト（画像＋アイテム）を追加できる広告タイプの商品です。
        * 日本語・英語を問わず、スペースを含むテキストの 1 番目のアイテムタイトル 25 文字、2〜4 番目のアイテムタイトル 30 文字＋画像アイテム 3〜4 個＋リンクボタン最大 2 個（横配列）
    * カルーセルフィード
        * 最大 10 枚の画像とさまざまなテキスト情報を含めることができる広告タイプの商品です。
        * 日本語・英語を問わず、スペースを含むタイトル 20 文字のテキスト＋文言 180 文字のテキスト＋画像＋リンクボタン 2 個（横配列）で構成されたアイテム最大 6 個
    * プレミアム動画
        * 添付した動画がメッセージバルーンに自動再生されるタイプです。
        * 動画リンクは、カカオ TV にアップロードされた動画のみ使用できます（例: [https://tv.kakao.com/v/#{숫자}](https://tv.kakao.com/v/#%7B%EC%88%AB%EC%9E%90%7D) / [https://tv.kakao.com/channel/#{숫자}/cliplink/#{숫자}](https://tv.kakao.com/channel/#%7B%EC%88%AB%EC%9E%90%7D/cliplink/#%7B%EC%88%AB%EC%9E%90%7D)）。
        * 日本語・英語を問わず、スペースを含むヘッダー 20 文字のテキスト＋文言 76 文字のテキスト＋カカオ TV にアップロードされた動画 1 本＋リンクボタン 1 個
    * コマース
        * 商品の価格や割引情報を強調して表示できるメッセージバルーンです。
        * 日本語・英語を問わず、スペースを含むタイトル 20 文字のテキスト＋補足情報 34 文字のテキスト＋リンクボタン最大 2 個（横配列）
    * カルーセルコマース
        * さまざまな商品の情報をカタログ形式で構成できるメッセージバルーンです。
        * 日本語・英語を問わず、スペースを含むタイトル 30 文字のテキスト＋補足情報 34 文字のテキスト＋リンクボタン最大 2 個（横配列）で構成されたアイテム最大 6 個
        * カルーセルコマースで使用するすべての画像は同一の比率である必要があります。
5. 画像がある場合は画像を選択します。
    * メッセージに画像を添付するには、先に **[画像管理]** タブで画像を登録する必要があります。
    * 画像リンク: 画像クリック時に遷移するリンクを入力します（http://、https:// を含む URL）。
    * カルーセルコマースで使用するすべての画像は同一の比率である必要があります。
6. Web リンク、アプリリンク、ボットキーワード、メッセージ転送、チャンネル追加、相談トーク転換、ボット転換、ビジネスフォームボタンを挿入できます。
    * 基本タイプ最大 5 個、カルーセル／ワイドアイテムリスト最大 2 個
7. メッセージでクーポンを強調する必要がある場合、ボタンを追加してクリック時に添付したクーポンに遷移するよう設定できます。
8. 代替送信メッセージの設定有無を選択します。
    * **[080 受信拒否管理]** タブで代替送信の使用を **[使用]** に設定し、SMS サービスを連携する必要があります。
9. 受信者を追加します。
    * 受信者は携帯電話番号の形式で入力できます。
10. 受信者ごとにターゲティングタイプを選択します。
    * ブランドメッセージは、広告主のマーケティング受信同意（以下「マーケティング同意」）ユーザーおよびチャンネル友だちユーザーを対象に送信できる広告性メッセージです。
    * ターゲティングタイプによって、指定した受信者へのメッセージ受信有無が異なる場合があります。
        * M: 広告主のマーケティング同意ユーザー（カカオトーク受信同意）
            * 広告主のマーケティング同意ユーザー（カカオトーク受信同意）を対象に広告性メッセージを送信します。
        * N: 広告主のマーケティング同意ユーザー（カカオトーク受信同意）- チャンネル友だち除外
            * 広告主のマーケティング同意ユーザー（カカオトーク受信同意）からチャンネル友だちを除外して広告性メッセージを送信します。
        * I: 広告主の送信リクエスト対象 ∩ チャンネル友だち
            * 広告主の送信リクエスト対象のうち、チャンネル友だちである場合にのみ広告性メッセージを送信します。
11. グループタグキーを入力します。（任意）
    * グループタグキーを指定すると、該当グループタグごとのカカオテンプレート統計を確認できます。
    * グループタグは **Notification > KakaoTalk Bizmessage > グループタグ管理** タブで登録できます。
