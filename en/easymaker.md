...
| ACTIVE (UNHEALTHY) | The notebook application is not running normally. If this status persists after restarting the notebook, contact support.      |
| STOPPED            | The notebook has been stopped.                                                                                                  |

### Saving Metric Logs for TensorBoard Usage

To view result metrics on the TensorBoard screen after training, you must set the TensorBoard log storage path to the designated location (`EM_TENSORBOARD_LOG_DIR`) when writing your training script.
