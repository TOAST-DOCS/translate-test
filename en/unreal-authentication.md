...
| -------------- | ---------------------------------------- | ---------- | ---------------------------------------- |
| Auth           | INVALID\_MEMBER                          | 6          | Invalid member request.                        |
|                | BANNED\_MEMBER                           | 7          | The member is banned.                               |
|                | AUTH\_USER\_CANCELED                     | 3001       | The login was canceled.                            |
|                | AUTH\_NOT\_SUPPORTED\_PROVIDER           | 3002       | Unsupported authentication method.                        |
...
|                | AUTH\_EXTERNAL\_LIBRARY\_ERROR           | 3009       | External authentication library error. <br/>Check the detailed error. |
...
|                | AUTH_TRANSFERACCOUNT_BLOCK               | 3042       | The account transfer feature has been locked because an invalid TransferAccount was entered multiple times. |
...
|  | AUTH_TOKEN_LOGIN_INVALID_LAST_LOGGED_IN_IDP | 3103 | No information about the last logged-in IdP. |
| IdP Login | AUTH_IDP_LOGIN_FAILED | 3201 | IdP login failed. |
|  | AUTH_IDP_LOGIN_INVALID_IDP_INFO | 3202 | The IdP information is not valid (the IdP information does not exist in the console). |
| Add Mapping | AUTH_ADD_MAPPING_FAILED | 3301 | Failed to add a mapping. |
...
|                | AUTH_ADD_MAPPING_CANNOT_ADD_GUEST_IDP    | 3305       | AddMapping is not available for guest IdPs. |
...
|                      | AUTH_ADD_MAPPING_FORCIBLY_EXPIRED_KEY           | 3313       | The forced mapping key (ForcingMappingKey) has expired. |
|                      | AUTH_ADD_MAPPING_FORCIBLY_DIFFERENT_IDP         | 3314       | The forced mapping key (ForcingMappingKey) was used for a different IdP. <br/>The issued ForcingMappingKey is used to attempt forced mapping with the same IdP. |
|                      | AUTH_ADD_MAPPING_FORCIBLY_DIFFERENT_AUTHKEY     | 3315       | The forced mapping key (ForcingMappingKey) was used for a different account. <br/>The issued ForcingMappingKey is used to attempt forced mapping with the same IdP and account. |
| Remove Mapping | AUTH_REMOVE_MAPPING_FAILED | 3401 | Failed to remove a mapping. |
|  | AUTH_REMOVE_MAPPING_LAST_MAPPED\_IDP | 3402 | The last mapped IdP cannot be removed. |
|  | AUTH_REMOVE_MAPPING_LOGGED_IN\_IDP | 3403 | This is the currently logged-in IdP. |
| Logout | AUTH_LOGOUT_FAILED | 3501 | Logout failed. |
| Withdrawal     | AUTH\_WITHDRAW\_FAILED                   | 3601       | Withdrawal failed.                              |
|                | AUTH\_WITHDRAW\_ALREADY\_TEMPORARY\_WITHDRAW | 3602   | The user has already requested a temporary withdrawal.                    | 
|                | AUTH\_WITHDRAW\_NOT\_TEMPORARY\_WITHDRAW | 3603       | The user has not requested a temporary withdrawal.                     | 
| Not Playable | AUTH_NOT_PLAYABLE | 3701 | Cannot play (maintenance or service ended, etc.). |
...
* The error information from the external authentication library is included in the detailed error. You can check the detailed error codes and messages as follows.
