...
| -------------- | ---------------------------------------- | ---------- | ---------------------------------------- |
| Auth           | INVALID\_MEMBER                          | 6          | This is a request for an invalid member.                        |
|                | BANNED\_MEMBER                           | 7          | This member has been banned.                               |
|                | AUTH\_USER\_CANCELED                     | 3001       | Login has been canceled.                            |
|                | AUTH\_NOT\_SUPPORTED\_PROVIDER           | 3002       | This authentication method is not supported.                        |
|                | AUTH\_NOT\_EXIST\_MEMBER                 | 3003       | This member does not exist or has withdrawn.                      |
|                | AUTH\_EXTERNAL\_LIBRARY\_INITIALIZATION\_ERROR | 3006 | Failed to initialize the external authentication library. |
|                | AUTH\_EXTERNAL\_LIBRARY\_ERROR           | 3009       | An external authentication library error occurred. <br/>Check the detailed error. |
...
|                | NOT\_GUEST\_OR\_HAS\_OTHERS              | 9          | You attempted to transfer from a non-guest account, or an IdP other than Guest is linked to the account. |
|                | AUTH_TRANSFERACCOUNT_EXPIRED             | 3041       | The validity period of the TransferAccount has expired. |
|                | AUTH_TRANSFERACCOUNT_BLOCK               | 3042       | The account transfer feature has been locked because an incorrect TransferAccount was entered multiple times. |
|                | AUTH_TRANSFERACCOUNT_INVALID_ID          | 3043       | The ID of the TransferAccount is not valid. |
|                | AUTH_TRANSFERACCOUNT_INVALID_PASSWORD    | 3044       | The password of the TransferAccount is not valid. |
|                | AUTH_TRANSFERACCOUNT_CONSOLE_NO_CONDITION | 3045      | TransferAccount has not been configured. <br/> Configure it first in the NHN Cloud Gamebase Console. |
|                | AUTH_TRANSFERACCOUNT_NOT_EXIST           | 3046       | TransferAccount does not exist. Issue a TransferAccount first. |
...
|  | AUTH_TOKEN_LOGIN_INVALID_LAST_LOGGED_IN_IDP | 3103 | There is no information about the most recently logged-in IdP. |
...
|                | AUTH_ADD_MAPPING_CANNOT_ADD_GUEST_IDP    | 3305       | AddMapping is not available for the Guest IdP. |
| Add Mapping Forcibly | AUTH_ADD_MAPPING_FORCIBLY_NOT_EXIST_KEY         | 3311       | The forced mapping key (ForcingMappingKey) does not exist. <br/>Check the ForcingMappingTicket again. |
|                      | AUTH_ADD_MAPPING_FORCIBLY_ALREADY_USED_KEY      | 3312       | The forced mapping key (ForcingMappingKey) has already been used. |
|                      | AUTH_ADD_MAPPING_FORCIBLY_EXPIRED_KEY           | 3313       | The validity period of the forced mapping key (ForcingMappingKey) has expired. |
|                      | AUTH_ADD_MAPPING_FORCIBLY_DIFFERENT_IDP         | 3314       | The forced mapping key (ForcingMappingKey) was used for a different IdP. <br/>The issued ForcingMappingKey is used to attempt forced mapping to the same IdP. |
|                      | AUTH_ADD_MAPPING_FORCIBLY_DIFFERENT_AUTHKEY     | 3315       | The forced mapping key (ForcingMappingKey) was used for a different account. <br/>The issued ForcingMappingKey is used to attempt forced mapping to the same IdP and account. |
...
|                | AUTH\_WITHDRAW\_ALREADY\_TEMPORARY\_WITHDRAW | 3602   | This user has already requested a temporary withdrawal.                    | 
|                | AUTH\_WITHDRAW\_NOT\_TEMPORARY\_WITHDRAW | 3603       | This user has not requested a temporary withdrawal.                     | 
| Not Playable | AUTH_NOT_PLAYABLE | 3701 | The game is in a state that cannot be played (under maintenance or service has ended, etc.). |
| Auth(Unknown) | AUTH_UNKNOWN_ERROR | 3999 | An unknown error has occurred. (This is an undefined error.) |

* For all error codes, refer to the following document.
    * [Error Codes](./error-code/#client-sdk)

**AUTH_EXTERNAL_LIBRARY_ERROR**

* This error is returned when an error occurs in the external authentication library.
* The error information from the external authentication library is included in the detailed error, and you can check the detailed error code and message as follows.
