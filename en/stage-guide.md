...
<!-- pre-align:aligned sig=af41331614af -->
...
For how to write a **Manifest**, see the [Kubernetes documentation](https://kubernetes.io/docs/concepts/workloads/controllers/deployment ).
...
If you disable **Use resource version management** in the **Deploy** stage, the resources deployed by that stage are deployed using the original names defined in the manifest, and the resource version management feature of the Pipeline service is not available. We recommend disabling this option only when resources are directly queried by their original names from outside the deployment manifest, such as by operators or controllers.
...
The resource version management option applies to all resources deployed by that stage. To configure specific resources differently, you can add the `strategy.spinnaker.io/versioned` annotation to `metadata.annotations` in the manifest with a value of "`true`" or "`false`" to set version management on a per-resource basis. Whether resource version management is applied is determined by the following priority order:

1. The `strategy.spinnaker.io/versioned` annotation on the resource
2. The **Use resource version management** setting in the Deploy stage

**Constraints**
* Resources with version management disabled do not retain a version history, so the rollback feature of the **Deploy - Rollout undo** stage and the **Manage Deployment Targets** workload cannot be used. Even if a rollback is performed, the previous ConfigMap and Secret configurations are not restored.
