...
To use the S3-compatible API, you must first issue S3 API credentials in the AWS EC2 format (Access Key ID + Secret Access Key). Credentials can be issued using the console or API. To obtain credentials using the console, refer to [S3 API Credentials](console-guide/#s3-api-credentials). For more information, refer to the [S3 API Credentials](s3-api-guide/#s3-api-credential) section of the Amazon S3-compatible API guide.
