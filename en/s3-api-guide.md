...
Object Storage uses S3 API credentials for authentication/authorization when making Amazon S3-compatible API calls. S3 API credentials are authentication keys in the AWS EC2 format used to access service APIs that support the Amazon S3-compatible API in NHN Cloud.
