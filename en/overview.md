...
| Feature | Supported Starting Version | Description |
| -- | -- | -- |
| HTTP/2 protocol support | v2 | You can choose between HTTP/1 and HTTP/2. Only HTTP/1 is supported in v1. |


!!! danger "Caution"
    Changing the engine version may alter how HTTP traffic is handled, as described below. Make sure to test thoroughly before applying the change to your production environment.

    * HTTP response chunk handling: `v2` can merge multiple chunks of an HTTP response that are sent separately into a single response. Clients that expect responses to arrive in chunks may behave differently.
    * HTTP header name format: `v2` may convert HTTP/1.1 header names in requests and responses to lowercase (for example, `Content-Type` → `content-type`). Although HTTP header names are case-insensitive by standard, backend servers or clients that treat header names as case-sensitive may be affected. In particular, clients that read response headers may be impacted.
    * HTTP standard compliance: `v2` enforces HTTP standards more strictly. If you use non-standard request or response formats, behavior may change.

    `v2` complies with the HTTP standard (RFC), but in doing so, some behaviors may differ slightly from the previous version (`v1`). The items listed above are representative examples, and other behaviors not explicitly mentioned may also change. After changing the engine version, make sure to perform sufficient validation before applying it to your production environment.
...
The load balancer operates strictly according to the selected protocol version. If you select HTTP/2, it cannot communicate using HTTP/1.
If you select HTTP or HTTPS as the health check protocol, the load balancer operates with the same protocol version as the one selected for the member group.

!!! danger "Caution"
    - This feature is not available in load balancer engine version v1.
    - If the member group protocol version is HTTP/2, the health check protocol is set to HTTP or HTTPS, and no Host is entered, `NHNLB` is automatically set in the Host header.
...
Therefore, when the health check protocol is HTTP or HTTPS and the proxy protocol is sent, the member instance must be able to recognize the proxy protocol in order to return a normal response and enter the ACTIVE state. If the member instance does not support the proxy protocol, set the health check port to **Specify** so that the proxy protocol is not sent.

!!! tip "Note"
    When the health check protocol is TCP, only the success of a TCP handshake with the member instance is verified. Therefore, regardless of whether the proxy protocol is sent or whether the member instance supports the proxy protocol, the instance is considered ACTIVE as long as the port is open.
...
If a proxy protocol is set on a listener, the health check behavior varies depending on the health check port settings. For more information, see "Proxy Protocol and Health Checks" in "Load Balancer Proxy Mode".
