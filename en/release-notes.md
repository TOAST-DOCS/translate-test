...
<!-- pre-align:aligned sig=b3be6c89d505 -->
...
* GPU-related (Linux)
    ...
    * Applied NVIDIA server driver package
    * Added CUDA toolkit: 12.6
    * DCGM: 4.5.0 → 4.6.0
    * DCGM-Exporter: 4.6.0 → 4.8.3
        * Changed to run as root instead of non-root for PROF metric collection
        * Excluded LOW_UTIL_VIOLATION metric collection due to negative counter issues depending on the GPU
        * Changed the listen address and port to 0.0.0.0:9400 to allow external collection of Exporter data
