...
| policy | Query | String | Y | Query parameter for retrieving a policy{% if release_2026_08 %}<br>If no value is specified, the entire policy document is retrieved. If a top-level key for a specific feature is specified, only the policy for that feature is retrieved.{% else %} (used without a value){% endif %} |
...
{% else %}
The policy is overwritten per top-level key included in the request (e.g., lifecycle), and the settings of keys not included are retained.
{% endif %}
...
|---|---|---|---|---|
| `default_rule` | Object | N | Basic rules |{% if not release_2026_08 %} If omitted or set to an empty object (`{}`), the default rule is removed.{% endif %} |
...
| `rules` | Array | N | List of condition rules | {% if release_2026_08 %} Max. 30 items{% else %} If omitted or set to an empty array (`[]`), all condition rules are deleted.{% endif %} |
