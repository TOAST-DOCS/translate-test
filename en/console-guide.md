...
{% if release_2026_05 %}For more information about lifecycle settings, see [How to Apply Lifecycle Rules](container-policy-guide/#lifecycle-apply).{% endif %}
...
{%- endif %}
    ...
    <td>Enter the object lifecycle in days.{% if not release_2026_05 %} Life cycle setting will be cleared if it is not filled in.{% endif %}</td>
  </tr>
  <tr>
...
!!! tip "Note"
    The modified life cycle rules are applied only to objects uploaded after the settings are changed.
{%- if release_2026_08 %}

You can set a maximum of 30 condition rules. This limit also applies when configuring via [Container Policy](container-policy-guide/#lifecycle).
{%- endif %}
{%- if ec %}
