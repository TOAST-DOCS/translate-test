...
## Creating CDN Service { #creating-cdn-service }
...
> - Protocol downgrade is not applied to the entire website address. For instance, www.nhn.com, which is the entire site address of the origin server, cannot be downgraded.
> - No other methods than `GET`, `HEAD`, or `OPTIONS`, are supported.
> - When a downgrade is requested from CDN to an origin server, following headers may be excluded:
>    `Origin`, `Referer`, `Cookie`, `Cookie2`, `sec-\*`, and `proxy-\*`
...
### Controlling the access of root path { #controlling-the-access-of-root-path }
...
### Access Control for Auth Token Authentication { #access-control-for-auth-token-authentication }
...
> - Create a token required to access content.
> - Client (final content consumer) must request content including created token.
...
### Modify CDN Service Setting { #modify-cdn-service-setting }
...
2. Click **OK** to complete with changes.

To edit other settings, except description and callback setting, it may take more time than usual since changes must be applied throughout the whole CDN server. Modification takes dozens of minutes, but change of domain alias may take a few hours.
...
### Suspend and Resume CDN { #suspend-and-resume-cdn }
...
The alias domain feature allows you to use a domain you own instead of the default [service ID].toastcdn.net domain provided by NHN Cloud CDN.
To use an alias domain, register a domain in the **Alias Domain** tab, complete ownership verification, and integrate it with the alias domain settings of your CDN service. To use HTTPS protocol with your own domain, issue a certificate in the **Certificate Management** tab.

> [Note] HTTPS service configuration flow using alias domain
> To use HTTPS protocol with your own domain, complete the following steps in order.
> If you only use HTTP, you can skip step ② certificate issuance.
> ```
> ① Domain registration and ownership verification (Alias Domain tab)
>         ↓
> ② Certificate issuance and domain verification (Certificate Management tab, required for HTTPS)
>         ↓
> ③ Integrate alias domain with CDN service (CDN Service tab)
>         ↓
> ④ Configure DNS CNAME record for CDN service domain (DNS service provider)
> ```
...
3. Click the **OK** button to register the domain and set it to awaiting verification status.

> [Caution] Notes on domain registration
> - Only fully qualified domain name (FQDN) format is accepted (e.g., cdn.example.com).
> - Domains that are already registered cannot be registered again.
...
1. Add a TXT record in the DNS management page of your DNS service provider.
   DNS configuration methods may vary depending on the DNS service provider. Contact your service provider for assistance with the configuration.
    - Record type: `TXT`
    - TTL: `60`. If 60 cannot be set, use as small a value as possible.
    - Record name: Enter the `Record name` value displayed in the console.
    - Record value: Enter the `Record value` displayed in the console.
2. After configuring the TXT record, click the **Run Verification** button in the console.

<a id="verify-a-domain-http-file-authentication-method"></a>
#### HTTP file authentication method
Verifies the domain by adding an HTTP page to the web server connected to the domain.

1. Create a page accessible from the web server at the `HTTP page URL` path displayed in the console.
2. Set the `Page content (token)` value displayed in the console as the body of the created page.
3. Access the URL from a web browser and confirm that the page content value is displayed.
4. After confirming, click the **Run Verification** button in the console.

> [Caution] The HTTP file authentication method can only be used on web servers operating on the default HTTP port (80).

<a id="verify-a-domain-http-redirect-authentication-method"></a>
#### HTTP redirect authentication method
Verifies the domain by configuring a redirect on the web server connected to the domain.

1. Configure an HTTP 301 or 302 redirect from the `Redirect source URL` to the `Redirect target URL` displayed in the console.
2. After configuring the redirect, click the **Run Verification** button in the console.

> [Caution] Notes on domain verification
> 1. The domain verification token is valid for 14 days from the date of registration. If verification is not completed within this period, the token will expire, and verification must be performed again after reissuing the token.
> 2. If the token has expired, click the **Reissue Token** button in the console to issue a new verification token. When the token is reissued, the previous verification information is reset, and verification must be performed again with the new token information.
> 3. When domain verification is complete, the status changes to **Verification complete**, and the domain can be integrated with the CDN service.

[Table] Alias domain verification status

| Status | Description |
|----------| ----------------------------------------------------- |
| Awaiting verification | Domain is registered and awaiting verification |
| Verification in progress | Domain ownership verification is in progress |
| Verification complete | Domain ownership verification is complete and the domain can be integrated with the CDN service |
| Verification token expired | The verification token has expired (reissue the token and verify again) |

<a id="integrate-an-alias-domain-with-the-cdn-service"></a>
### Integrate an alias domain with the CDN service { #integrate-an-alias-domain-with-the-cdn-service }
...
2. Select the verified domain in the **Alias Domain** field under **Basic Information**.
    - Only verified domains can be selected. If the domain you want to configure is not displayed in the list, register the domain and complete verification in the **Alias Domain** tab first.
3. Click the **OK** button to complete the changes.
4. After configuring the alias domain, configure the CNAME record with your DNS service provider as follows. For questions about DNS configuration, contact your DNS service provider.
    - Record type: `CNAME`
    - Record name: `[Domain registered as alias domain]`
    - Record value (Rdata): `[Service ID].toastcdn.net`
    - TTL: Any value

> [Note] Time required for alias domain changes
> When an alias domain is changed, the service modification process may take over 1 hour.

> [Caution] Certificate notes when changing alias domain
> If a CDN certificate domain is removed from the alias domain, the certificate may expire.
> To keep the certificate active, add the alias domain again and authenticate it, then re-integrate it as an alias domain with the CDN service.
...
> [Caution] Notes on domain deletion
> Domains integrated with the CDN service cannot be deleted. To delete a domain, first unlink the alias domain from the CDN service, then delete it.


<a id="certificate"></a>
## Managing Certificates { #certificate }
...
You're ready to validate domain, after a new certificate is requested, when the certificate is changed to 'Validate Domain'.
...
> - Domain must be validated within 5 days since when a certificate is requested of issuance. Otherwise, getting a certificate shall be automatically revoked.
> - When domain is successfully validated, certificate is to be issued and deployed within hours. Unless it proceeds more than a day, check if domain has been properly validated. If it still does not proceed, contact [NHN Cloud Customer Center](https://www.nhncloud.com/kr/support/inquiry).
> - Adding HTTP Pages is available only when the HTTP server runs on the default (80) ports. If port change is unavailable, please take another option of Adding DNS TXT Records.

<a id="issue-and-deploy-certificates"></a>
### Issue and Deploy Certificates { #issue-and-deploy-certificates }
...
### Integrate with CDN Service { #integrate-with-cdn-service }
...
>
> - Assign the domain of a certificate to the domain address of CDN service which is to be integrated through CNAME record.
> - Set certificate domain for domain alias of CDN service to be integrated with.
> - When a CDN service integrated with a certificate is suspended, the certificate cannot be renewed.
     >    - Resume before a renewal start date or integrate it to another running CDN service.
> - When a CDN service integrated with a certificate is deleted, the certificate cannot be renewed.
     >    - Integrate it to another running CDN service before deleting.
> - The nameserver (NS) and DNS settings of the certificate domain must be maintained normally.
...
> - ISRG x1 certificate download link: [Download link](https://letsencrypt.org/certs/isrgrootx1.pem)
> - Windows OS settings change reference guide: [Link](https://docs.microsoft.com/en-us/skype-sdk/sdn/articles/installing-the-trusted-root-certificate)
> - Chrome browser reference guide: [Link](https://docs.vmware.com/en/VMware-Adapter-for-SAP-Landscape-Management/2.0.1/Installation-and-Administration-Guide-for-VLA-Administrators/GUID-D60F08AD-6E54-4959-A272-458D08B8B038.html)
