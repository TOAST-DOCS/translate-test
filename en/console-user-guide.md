...
All dates and times displayed on the screen are based on the time zone of the browser that you are using to access the service.
...
!!! danger "Caution"
    ...
    This notice is also displayed as a pop-up when you enter the console. If you choose **Don't show again**, it will not be displayed again in the same browser, but it will be displayed again in a different browser or a different project.
...
| Item | Required | Description |
| --- | --- | --- |
| Data Source Type | O | Select the method for loading data. The default is File Upload. |

See the table below for available types and their descriptions:
...
In the detail settings, you specify the following two items.
...
Below the Group Label, the **Preview** row shows how the sample metric is divided into series and groups based on the label settings you entered, in the format "N series · M groups." If you leave the Group Label empty, it displays "1 group · The entire data source is one group."

- Click **View Example** to expand a table that divides series by group and a request body example. Click a row in the table to update the request body example with the label for that series.
- If you **manually specify** a series identification label and multiple metrics are merged into one series, the number of merged metrics is displayed.
- Any changes to the input are recalculated immediately.

Clicking the **Add** button opens a completion window that displays the data source readiness status and the **collection method** (endpoint, request headers, request body example, and rules). Once the status changes to "Ready," you can send metrics using the provided method.

!!! tip "Tip"
    For how to send data to the Prometheus API data source, see the creation completion window, the **Collection Method** tab in View Details, or 'Metrics Ingest' in the [API Guide](../api-guide/#metrics.ingest.api).
...
- You cannot delete a data source if a data loading task is in progress or if another task is in progress on the data source. Try again after the task is complete.

- A data source in use by an app or a data pipeline cannot be deleted. Delete the associated app or data pipeline first.
- Deleting an app does not delete the inference result data source; it remains intact. If necessary, delete it separately from the Data Source List.
...
For data sources of the Prometheus API type, the **Series Identification Label** and **Group Label** are displayed together on the Connection information tab. If no value has been specified, they are displayed as "Use all labels" and "Single group (entire data source)", respectively. If multiple different values have come in per minute for the same time series within the last 5 minutes, that fact is also displayed on the Connection information tab.
...
| Enabling | Resources required for event collection are being created. The status is automatically updated when the process is complete. |
| ON | Event collection is enabled. |
| Failed | Activation has failed. Click the toggle again to retry. |

- The data source must be in the `COMPLETED` state before you can configure this setting. In any other state, the current state is displayed along with a **not settable** notice.
...
| Request header | `X-NC-APP-KEY` header populated with the app key for this data source and `X-NHN-Authorization` header for the authorization token |
| Request body example | Example request body that reflects the group label of this data source |
| Rule | Notes related to authorization tokens, time units, transmission intervals, labels, additional information, values, and responses |

The same information is also displayed in the completion window immediately after creating the data source.

!!! danger "Caution"
    The token field in the `X-NHN-Authorization` header must contain the issued authentication token. If you send only the app key, the request will be rejected.
...
- You can add up to 20 rows at a time by using the **Add Row** button.
...
| Start Date | Scheduling Started At |
| End date | Schedule end date |
| Last Executed Date | Most recent execution date and time |
...
- **Tab bar (right)**: Recent run date, status/build/activation badges, and Save button
...
The comparison value can be selected from **Enter Manually**, **Time (relative to now)**, **Another column**, or **Function/Expression**.

- **Function/Expression**: Enter a constant expression. Use this to filter a time column stored as a millisecond epoch value based on the current time. Example: `CAST(UNIX_TIMESTAMP(NOW() - INTERVAL 7 DAY) * 1000 AS BIGINT)`
- In Function/Expression, you can use only time calculation and type conversion functions, CAST target types, and INTERVAL units. Column references, quotation marks, semicolons, comments, and values exceeding 500 characters are rejected when saving.
...
| Query Executed Date | Date on which the query was executed |
...
| Chart Type | Chart Usage Category |
| Visualization Type | Chart Visualization Type |
| Data source name | Data source name used by the chart |
| Group Value | Column specified to divide and view by value |
| Created on | Date and time when the chart was created |
| Change Date | Last modified date and time of the chart |
...
- If there are columns with no aggregate function selected, a notification is displayed when you click **UPDATE CHART**. Without an aggregate function, the chart may not be displayed normally.
...
- You can change the default settings and query settings.
...
| Failed to verify the resource status. | Evaluation failed. If resources are insufficient, the app may not start normally. |

- If resources are insufficient, clean up existing apps or apply for additional resources by submitting a 1:1 inquiry.
...
- If there is no data in the selected data source, 'No data yet.' is displayed below the item, and a **Check Data** box is displayed below Resource Check at the bottom of the screen. If you did not specify a retraining cycle, the training at creation is the only training, so you cannot proceed to the next step until data arrives. If you specified a retraining cycle, a message is displayed indicating that the first training will fail and will be retried at the next retraining, and you can proceed.
- If multiple different values arrive within the same time series in a single minute within the last 5 minutes, an information box is displayed. This does not prevent creation, but in this state, only the first value that arrives each minute is used for analysis, and the rest are discarded. Send data for the same time series only once per minute, and if you collect data at shorter intervals, aggregate it into a 1-minute average before sending.
...
| Item | Required | Description |
| --- | --- | --- |
| CPU Limit | O | The CPU upper limit allocated to the inference server. Uses Kubernetes notation. Examples: 2, 500m. Minimum 2 vCPU, default 2. |
| Memory Limit | O | The memory upper limit allocated to the inference server. Uses Kubernetes notation. Examples: 1Gi, 512Mi. Minimum 1Gi, default 1Gi. |
...
| Item | Required | Description |
| --- | --- | --- |
| Retraining Period | X | Toggle to enable or disable. Default is off. When enabled, specify the period below. |

| Period | Configuration Items |
| --- | --- |
| Daily | Hour and minute. Hour is from 0 to 23, minute is in 10-minute increments, and the default is 03:00. |
| Weekly | Day of week, hour, and minute. |
| Hourly Interval | Interval and minute. Select the interval from 1, 2, 3, 4, 6, 8, or 12 hours. |

- The configured time is applied based on the time zone of the browser you are using.
- If no cycle is specified, training is performed only once when the app is created, and automatic retraining is not performed afterward. In this case, you cannot create an app with a data source that has no data.
...
| Score Scale | X | Toggle for **Export on a 0–100 (%) scale**. Default is off. |
| Device | O | The computing device to use for inference and training. Select CPU or GPU; the default is CPU. |
...
- The app groups metrics in 1-minute intervals and makes a judgment for each time series. The data source connected to the app must continuously send data for the same time series at intervals of 1 minute or less. If data is sent at longer intervals, gaps will occur and preparation may not complete in precise mode. If multiple values are sent within 1 minute, only the first value to arrive is used for judgment. Loading metrics into the data source is unrelated to the transmission interval.
...
| Item | Required | Description |
| --- | --- | --- |
| Transmission URL | O | The address to which results are sent. |
| API Path | O | The path appended to the transmission URL. Default is /api/v1/write. |
| Score Metric Name | O | The metric name under which anomaly scores are stored. Default is AD_SCORE. |
| Threshold Metric Name | X | The metric name under which automatically calculated thresholds are stored. Default is AD_AUTO_THRESHOLD. |

Click **Expand Additional Transmission Settings** to configure the following items:

| Item | Required | Description |
| --- | --- | --- |
| Static Headers | X | HTTP headers that are always attached to transmission requests. Used for inserting authentication tokens. |
...
- Even if the transmission to the sending URL fails, it is not displayed in the console. If the results are not visible in the recipient Prometheus, compare them with the results stored in the result data source.
...
The schema of the Result Data Source is as follows. You can query these columns from the queries or charts in the Analysis menu.

| Field Name | Data Type | Description |
| --- | --- | --- |
| labelHash | string | Time series identifier. Same value as the labelHash in the metric data source |
| identityLabels | string | Original labels of the time series |
| score_aggregate | double | Anomaly score |
| threshold | double | Threshold for anomaly determination |
| eventTimestamp | long | Timestamp of the metric being evaluated. Millisecond epoch |
...
- If no retraining cycle is specified, it is displayed as 'Not specified (train once at creation)'.
...
When the univariate anomaly detection app is complete, a modal appears to guide you on what happens next.

- The model is trained and deployed. You can check the progress in the status column of the app list.
- In Exact mode, results do not begin to be sent until enough metric data has accumulated for judgment, even after activation. This typically takes several hours.
- During this time, you must continue sending metrics.

<a id="app.delete"></a>
### Delete App { #app.delete }

1. Select the checkbox of the app to delete. Only one app can be selected at a time, and selecting a different app deselects the previous selection.
2. Click the **Delete** button.
...
!!! danger "Caution"
    ...
    The inference result data source is not deleted and remains, so delete it separately from the Data Source List if necessary. The metric data source in use by the app can be deleted after the app is deleted.
...
Use the buttons at the top to perform the following actions. The actions apply to all training models included in the app.
...
| Situation | Message |
| --- | --- |
| No training model exists | There are no training models. |
...
| Usage Status | The artifact currently used in serving is displayed as **Serving**, and the most recent artifact is displayed as **Latest Training** |
...
| Group Key Field | Label used to divide groups. Displays the group label settings of the data source. If not specified, displays 'Disabled (single group)' |
...
| Retraining Interval | In the format 'Every day at 03:00', 'Every Monday at 03:00', or 'Every 6 hours'. If not specified, displays 'No automatic retraining (trained once at creation)' |
| Transfer Mode | Accurate mode, Immediate mode |
| Score Scale | Raw value or 0–100 scale |

**Output**: Result transfer

| Item | Description |
| --- | --- |
| Transfer Address | The address to which results are sent. The value is a combination of the transfer URL and the API path |
| Score Metric Name | The metric name where anomaly scores are stored |
| Threshold Metric Name | The metric name where the anomaly detection threshold value is stored |
...
- You can check the meaning of the three states by hovering the mouse over the question mark icon next to the group status title. Activation Pending usually takes several hours in precise mode, while instant mode activates immediately after the group is turned on.
- If retraining fails, the training status is displayed as Training Failed. Results continue to be sent using the most recently trained model, and training is attempted again at the next retraining cycle. If the initial training fails, the app enters a Failed state and can be deleted.
...
| Column | Description |
| --- | --- |
| Group Key | The label name used as the basis for dividing groups. Displayed as a hyphen for apps that have no group key field |
...
| Activation Time | The time when enough data was collected for judgment and results started being sent. Displays a hyphen for groups that are pending activation. |
| Deactivation Time | The time of the last deactivation. This value is retained even after reactivation, preserving the previous history. Displays a hyphen if the group has never been deactivated. |
...
- Activation Pending usually takes several hours in Exact mode. Immediate mode activates right after you turn on the group.
...
- You can sort the Group Key, Activation Time, Deactivation Time, Created On, and Modified On columns by clicking the column header. Sorting applies to all groups, and changing the sort order navigates to page 1. The Value, Group Hash, and Status columns cannot be sorted; values are sorted under the Group Key column.
...
- To start, stop, or delete specific groups selectively, see "Start, Stop, and Delete Groups" in the [API Guide](../api-guide/#univariate.group.api). This operation is not available in the console.
...
- The canonical form and the 16-character hash are displayed, and you can copy them using the **Copy** button.
