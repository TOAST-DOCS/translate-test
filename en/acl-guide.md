...
{% if release_2026_08 %}The HTTP referer allow/block access policy applies the block policy first, regardless of the input order. Therefore, access requests from an HTTP referer designated as a block target are denied even if the `.r:*` policy element that allows all access is also entered.{% endif %}
