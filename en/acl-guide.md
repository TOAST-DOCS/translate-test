...
## Storage > Object Storage > Access Policy Configuration Guide { #storage-object-storage-acl-configuration-guide }

This document describes how to set role-based access policies and IP-based access policies on NHN Cloud Object Storage containers.
...
!!! tip "Tip"
    The maximum number of access policy elements that can be set for `X-Container-Read`, `X-Container-Write`, and `X-Container-View` is 100 per property. This limit also applies when configuring via the [container policy](container-policy-guide/#acl).
...
!!! tip "Tip"
    You can find `<api-user-id>` by referring to the **API User ID** field in the API Endpoint setting dialog box on the console, or in the **access.user.id** field of the authentication token issuance API response body.
...
!!! tip "Note"
    Values with a colon on one side empty, such as `<tenant-id>:` or `:<api-user-id>`, and values that start with `.` cannot be used.
...
!!! tip "Note"
    In the referrer, `*` can only be used as `.r:*`, which means full public access. Values that combine `*` with other characters, `.r:-*` which blocks all access, and empty values cannot be used.
...
The HTTP referer allow/block policy applies the block policy first, regardless of the order in which the policies are entered. Therefore, access requests from an HTTP referer designated as a block target are denied even if you also enter the `.r:*` policy element that allows all access.
...
!!! danger "Caution"
    If you do not have read permission, you can no longer operate the container from the console.
...
You can use the API to enable IP-based ACL by entering IP-based access policy elements in the `X-Container-Ip-Acl-Allowed-List` and `X-Container-Ip-Acl-Denied-List` properties of a container. `X-Container-Ip-Acl-Allowed-List` is a whitelist, and `X-Container-Ip-Acl-Denied-List` is a blacklist.

!!! tip "Note"
    The maximum number of policy elements that can be set in `X-Container-Ip-Acl-Allowed-List` (whitelist) and `X-Container-Ip-Acl-Denied-List` (blacklist) is 100 each. This limit also applies when setting via [container policy](container-policy-guide/#ip-acl).
