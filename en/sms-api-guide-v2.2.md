...
* If registration date and delivery date are queried simultaneously, the delivery date is ignored.
...
| msgStatus            | 	String  | 1     | 	Optional | Message status code (0: Failure, 1: Request, 2: Processing, 3: Success, 4: Reservation canceled, 5: Duplicate failure, 6: Failure (advertising restriction), 7: Waiting for resend (advertising restriction)) |
...
- Closing phrase: `무료수신거부 {080수신거부번호}` or `무료거부 {080수신거부번호}`
  ...
  - A hyphen (-) may be included between the digits of the 080 opt-out number.
