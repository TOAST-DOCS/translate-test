...
* If you query both the registration date and delivery date simultaneously, the delivery date is ignored.
...
| recipientNo          | String  | 20         | Optional | Recipient number                                                                                                                                                                    |
| templateId           | String  | 50         | Optional | Template number                                                                                                                                                                    |
| msgStatus            | String  | 1          | Optional | Message status code (0: Failure, 1: Requested, 2: Processing, 3: Success, 4: Reservation cancellation, 5: Duplicate failure, 6: Failure (ad restriction), 7: Waiting for resend (ad restriction)) |
| resultCode           | String  | 10         | Optional | Reception result code [[code table](./error-code/#_2)]                                                                                                                             |
| subResultCode        | String  | 10         | Optional | Detailed reception result code [[code table](./error-code/#_3)]                                                                                                                    |
...
[Request Body]
Same as the SMS sending described above.
[[See Request Body](./api-guide/#sms_2)]

<span style="color:red">However, the body must include mandatory advertising phrases.</span>

You can check the 080 number on the **080 Opt Out Setting** tab in the console.

The rules for mandatory advertising phrases are as follows:
- Starting phrase: `(광고)`
- Ending phrase: `무료수신거부 {080수신거부번호}` or `무료거부 {080수신거부번호}` 
  - The phrase may contain spaces.
...
- Closing phrase: `무료수신거부 {080수신거부번호}` or `무료거부 {080수신거부번호}`
  - The phrase may contain spaces.
  - A hyphen ('-') may be included in the 080 opt-out number.
