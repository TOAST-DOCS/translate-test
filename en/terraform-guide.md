...
<a id="terraform"></a>
## Terraform
Terraform is an open source tool that allows you to easily build infrastructure, make changes safely, and efficiently manage infrastructure configuration. The main features of Terraform are as follows:
...
* **Change Automation**
    ...
    * You can save time on building infrastructure and reduce mistakes.
...
Terraform executes by starting with the terraform binary file and calling desired targets in local environments or remote environments such as deployment servers. At this time, although the calling methods differ for each 'desired target', they interact by calling APIs provided by the target's supplier, namely the provider. Here, what enables Terraform to interact with targets is the 'provider'.
...
* **password**
    ...
    * For information on how to set the API password, refer to **User Guide > Compute > Instance > API Preparation**.
* **auth_url**
...
Write tf files in the path where the provider configuration file is located. You can collect multiple resource configurations in one tf file, or write separate tf files for each resource. Terraform reads all the written tf files at once to establish a construction plan.
