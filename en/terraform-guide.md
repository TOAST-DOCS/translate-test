...
NHN Cloud is an official partner of HashiCorp and provides Terraform providers through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform is executed by calling a desired target in your local environment or a remote environment such as a deployment server, starting with the Terraform binary file. The desired targets use different invocation methods, but they interact by calling APIs provided by the target's provider. A "provider" is what enables Terraform to interact with the target.
...
Infrastructure construction using Terraform typically follows a lifecycle as described below:
...
First, write the infrastructure configuration in a tf file. To verify the construction plan according to the written tf file, use the `plan` command as follows:

```
$ terraform plan
```

If the construction plan is satisfactory, use the `apply` command to create, modify, or delete resources.
...
Write the tf file in the path where the provider configuration file is located. You can consolidate multiple resource configurations in a single tf file, or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a construction plan.

Below is an example of a tf file that defines resources to create an instance in the `instance.tf` file.
