...
* **Infrastructure as Code**
    ...
    * The defined code can be easily shared, enabling efficient collaboration.
* **Execution Plan**
    * By separating the change plan from the change application, you can reduce mistakes that can occur when applying changes.
* **Resource Graph**
    * You can preview how minor changes will impact your entire infrastructure.
    * By creating a dependency graph, you can plan based on this graph and preview how the infrastructure state will change when the plan is applied.
* **Change Automation**
    * You can automate building and changing infrastructure with the same configuration in multiple locations.
    * This saves time spent building infrastructure and reduces mistakes.
...
Download the file that matches your local PC's operating system from the [Terraform Download page](https://www.terraform.io/downloads.html). Decompress the file, place it in your desired path, and then add that path to your environment settings to complete the installation.
...
Refer to the `VERSION` information in the [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest) to determine the provider version.
...
* **user_name**
    * Uses NHN Cloud ID.
* **tenant_id**
    * Check the tenant ID by clicking the **API Endpoint Settings** button in the **Compute > Instance > Manage** menu in the NHN Cloud console.
* **password**
...
* **auth_url**
    ...
    * Check the identity service URL by clicking the **API Endpoint Settings** button in the **Compute > Instance > Manage** menu in the NHN Cloud console.
* **region**
