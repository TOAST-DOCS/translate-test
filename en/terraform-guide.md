...
NHN Cloud, as an official partner of HashiCorp, provides Terraform provider through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform executes by starting with a terraform binary file and calling the desired target in a local environment or in a remote environment such as a deployment server. At this time, the desired target has different calling methods, but they interact by calling the API provided by the target's provider. The 'provider' is what enables Terraform to interact with the target.
...
Infrastructure construction with Terraform typically follows the lifecycle shown below.
...
First, write the infrastructure configuration you want to build in a tf file. You can check the construction plan based on the written tf file using the `plan` command as shown below.

```
$ terraform plan
```

If the construction plan is without issues, use the `apply` command to create, modify, or delete resources.
...
Write a tf file in the path where the provider configuration file is located. You can either collect multiple resource configurations in a single tf file or write them in separate tf files for each resource. Terraform reads all the written tf files at once and establishes a construction plan.
