...
NHN Cloud, as an official HashiCorp partner, provides a Terraform provider through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform is executed by starting with the terraform binary file and calling desired targets in a local environment or remote environments such as deployment servers. In this process, desired targets have different invocation methods but interact by calling APIs provided by the target's provider. A 'provider' is what enables Terraform to interact with targets.
...
First, write the infrastructure configuration you want to build in a tf file. You can review the build plan based on the written tf file using the `plan` command as follows:
...
Write tf files on the path where the provider configuration file is located. You can collect multiple resource configurations in a single tf file, or write separate tf files for each resource. Terraform reads all written tf files at once to establish a build plan.

Below is an example of a tf file that defines a resource for creating an instance in an `instance.tf` file:
