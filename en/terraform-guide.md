...
<a id="terraform"></a>
## Terraform
Terraform is an open source tool that enables you to easily build infrastructure, safely make changes, and efficiently manage infrastructure configuration. The main features of Terraform are as follows:
...
* **The names and numbers of components, including versions, may change, so please check before use.**
...
Terraform starts with the terraform binary file and runs by calling desired targets in local environments or remote environments such as deployment servers. At this time, these 'desired targets' have different calling methods, but they interact by calling APIs provided by the target's supplier, namely the provider. Here, what enables Terraform to interact with targets is the 'provider'.
...
Infrastructure construction using Terraform typically follows a lifecycle as shown below:
...
First, write the infrastructure configuration to be built in tf files. Review the construction plan based on the written tf files using the `plan` command as follows:
...
Write tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in one tf file, or create separate tf files for each resource. Terraform reads all written tf files at once to establish a construction plan.
