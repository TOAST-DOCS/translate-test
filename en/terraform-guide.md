...
Terraform runs by starting with the terraform binary file and calling the desired target in either a local environment or a remote environment such as a deployment server. The desired target has different invocation methods, but it interacts by calling the API provided by the target's provider. A 'provider' enables Terraform to interact with the target.
...
First, write the infrastructure configuration to be deployed in a tf file. Based on the written tf file, check the deployment plan using the `plan` command as follows:

```
$ terraform plan
```

If the deployment plan is satisfactory, use the `apply` command to create, modify, or delete resources.
...
Write tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file or write them in separate tf files by resource. Terraform reads all written tf files at once and establishes a deployment plan.
