...
* **Execution Plan**
    * You can separate the change plan from the change application, reducing mistakes that may occur when applying changes.
* **Resource Graph**
    * You can preview how minor changes will affect the entire infrastructure.
    * You can create a dependency graph and make plans based on this graph, and verify the infrastructure state that changes when the plan is applied.
* **Change Automation**
...
Download the file that matches your local PC's operating system from the [Terraform download page](https://www.terraform.io/downloads.html). Extract the file, place it in the desired location, and add that path to your environment settings to complete the installation.
...
* **tenant_id**
    * Click the **API Endpoint Setting** button in the **Compute > Instance > Management** menu of the NHN Cloud console to check your tenant ID.
* **password**
    * Use the **API password** saved in the **API Endpoint Setting** dialog.
    * Refer to **User Guide > Compute > Instance > API Usage Preparation** for instructions on how to set the API password.
* **auth_url**
