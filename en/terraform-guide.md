...
* **Execution Plan**
    * You can separate the change plan from the change application to reduce mistakes that may occur when applying changes.
* **Resource Graph**
    * You can see in advance how minor changes will affect the entire infrastructure.
    * You can create a dependency graph and make a plan based on this graph, and confirm the changed infrastructure state when this plan is applied.
* **Change Automation**
    * You can automate the creation and modification of infrastructure with the same configuration in multiple locations.
...
Download the file that matches your local PC's operating system from the [Terraform Download Page](https://www.terraform.io/downloads.html). Unzip the file, place it in your desired path, and add that path to your environment settings to complete the installation.
...
* **tenant_id**
    * Click the **API Endpoint Settings** button in the **Compute > Instance > Management** menu in the NHN Cloud console to verify your tenant ID.
* **password**
...
* **auth_url**
    ...
    * Click the **API Endpoint Settings** button in the **Compute > Instance > Management** menu in the NHN Cloud console to verify the identity service URL.
* **region**
