...
## Third-Party Guides > Terraform User Guide
This document describes how to use NHN Cloud with Terraform.

<a id="terraform"></a>
## Terraform
Terraform is an open-source tool that makes it easy to build infrastructure, safely change it, and efficiently manage infrastructure configuration. Key features of Terraform include the following:

* **Infrastructure as Code**
    * You can define infrastructure as code to improve productivity and transparency.
    * Defined code can be easily shared, enabling efficient collaboration.
* **Execution Plan**
...
* **Resource Graph**
    ...
    * You can create a dependency graph, make plans based on this graph, and check the infrastructure state changes when the plan is applied.
* **Change Automation**
...
* **The names and numbers of components, including versions, may change. Please check before using.**


<a id="terraform-installation"></a>
## Terraform Installation
Download the file that matches your local PC's operating system from the [Terraform Download page](https://www.terraform.io/downloads.html). Extract the file, place it in your desired path, and then add that path to your environment settings to complete the installation.
...
Refer to the `VERSION` information in the [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest) to write the provider version.
...
* **tenant_id**
    * In the NHN Cloud console, go to **Compute > Instance > Manage** menu and click the **API Endpoint Setting** button to verify the tenant ID.
* **password**
    * Use the **API password** saved in the **API Endpoint Setting** dialog.
    * For information on how to set the API password, refer to **User Guide > Compute > Instance > Prepare for API Use**.
* **auth_url**
    * Specify the NHN Cloud identity service address.
    * In the NHN Cloud console, go to **Compute > Instance > Manage** menu and click the **API Endpoint Setting** button to verify the identity service URL.
* **region**
