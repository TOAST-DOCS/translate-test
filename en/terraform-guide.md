...
Terraform starts with the terraform binary file and runs by calling desired targets in a local environment or a remote environment such as a deployment server. The "desired targets" are called in different ways, but they interact by calling APIs provided by the target's supplier, which is the provider. Here, the "provider" is what enables Terraform to interact with those targets.
