...
NHN Cloud, as an official partner of HashiCorp, provides Terraform providers through the [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform runs by calling desired targets in local environments or remote environments like deployment servers, starting with the terraform binary file. The desired targets use different calling methods, but they interact by calling APIs provided by providers. A provider enables Terraform to interact with the target.
...
Infrastructure deployment using Terraform typically follows the lifecycle below:
...
First, write the infrastructure configuration to tf files. Verify the deployment plan for the written tf files using the `plan` command as follows:

```
$ terraform plan
```

If the deployment plan looks correct, use the `apply` command to create, modify, or delete resources.
...
Write tf files in the path where the provider configuration file is located. You can collect multiple resource configurations in one tf file or write separate tf files for each resource. Terraform reads all the written tf files at once and establishes a deployment plan.

The following is an example of a tf file that defines resources to create an instance in the `instance.tf` file.
