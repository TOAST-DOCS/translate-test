...
As an official partner of HashiCorp, NHN Cloud provides a Terraform provider through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform runs by invoking desired targets in local environments or remote environments such as deployment servers, starting with the terraform binary file. At this point, the 'desired targets' have different invocation methods, but they interact by calling APIs provided by the target's provider. Here, the 'provider' is what enables Terraform to interact with the target.
...
First, write the infrastructure configuration to be built in a tf file. Review the construction plan for the written tf file using the `plan` command as shown below.

```
$ terraform plan
```

If the construction plan is acceptable, use the `apply` command to create, modify, or delete resources.

```
$ terraform apply
```

The following section describes these steps in more detail with examples.


<a id="create-tf-files"></a>
### Write tf Files

Write tf files in the path where the provider configuration file is located. You can collect multiple resource configurations in one tf file or write them as separate tf files for each resource. Terraform reads all written tf files at once to create a construction plan.

Below is an example of a tf file that defines a resource to create an instance in the `instance.tf` file.
