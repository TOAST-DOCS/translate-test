...
<a id="terraform"></a>
## Terraform
Terraform is an open source tool that makes it easy to build infrastructure, safely apply changes, and efficiently manage infrastructure configuration. The key features of Terraform are as follows.
...
* **Resource Graph**
    ...
    * Create a dependency graph, develop plans based on this graph, and verify the infrastructure state changes when the plan is applied.
* **Change Automation**
    * Automate the ability to build and modify infrastructure with the same configuration across multiple locations.
...
Download a file that matches your local PC's operating system from the [Terraform download page](https://www.terraform.io/downloads.html). Extract the file, place it in your desired path, and then add that path to your environment settings to complete the installation.
...
The provider version is written by referring to the `VERSION` information in [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).
...
* **password**
    ...
    * For information on how to set the API password, refer to **User Guide > Compute > Instance > Preparing to Use the API**.
* **auth_url**
