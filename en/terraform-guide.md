...
NHN Cloud, as an official partner of HashiCorp, provides a Terraform provider through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform runs by starting with the terraform binary file and calling a desired target in a local environment or remote environment such as a deployment server. At this time, the desired targets have different invocation methods, but they interact by calling APIs provided by the target supplier, or provider. In Terraform, a 'provider' is what enables interaction with the target.
...
1. Write tf files
2. Verify construction plan
3. Create resources
4. Modify resources
5. Delete resources

First, write the infrastructure configuration to a tf file. Verify the construction plan from the written tf file using the `plan` command as follows:

```
$ terraform plan
```

If the construction plan is acceptable, create, modify, or delete resources using the `apply` command:
...
Write tf files at the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file, or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a construction plan.

The following is an example of a tf file that defines resources for creating an instance in an `instance.tf` file:
