...
As an official partner of HashiCorp, NHN Cloud provides a Terraform provider through the [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform runs by invoking a desired target starting from the terraform binary file in a local environment or a remote environment such as a deployment server. The desired target calls the API provided by the target's provider to interact with it. The 'provider' is what enables Terraform to interact with the target.
...
Infrastructure built with Terraform typically follows the lifecycle below:
...
First, write the desired infrastructure configuration in tf files. Review the build plan for the written tf files using the `plan` command as shown below:

```
$ terraform plan
```

If the build plan is acceptable, create, modify, or delete resources using the `apply` command:
...
Write tf files in the path where the provider configuration file is located. You can either collect multiple resource configurations in a single tf file or write separate tf files for each resource. Terraform reads all written tf files at once to establish a build plan.

The following is an example of a tf file that defines a resource for creating an instance in an `instance.tf` file:
