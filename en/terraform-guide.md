...
Terraform is executed by calling desired targets in local or remote environments such as deployment servers, beginning with the terraform binary file. Although these 'desired targets' are invoked differently, they interact by calling APIs provided by the target's provider. A 'provider' is what enables Terraform to interact with targets.
...
2. Verify the infrastructure plan
3. Create resources
4. Modify resources
5. Delete resources
