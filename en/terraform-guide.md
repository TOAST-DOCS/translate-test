...
* **Resource Graph**
    * Preview how minor changes might impact the entire infrastructure.
    * Create a dependency graph, develop a plan based on this graph, and check the changed infrastructure state when the plan is applied.
* **Change Automation**
    * Automate the construction and modification of infrastructure with the same configuration across multiple locations.
...
Below is an example of installation on Linux (Ubuntu/Debian).
...
* **tenant_id**
    * Click the **API Endpoint Settings** button in the NHN Cloud Console's **Compute > Instance > Management** menu to check the tenant ID.
* **password**
    * Use the **API Password** saved in the **API Endpoint Settings** dialog.
    * For instructions on setting the API password, refer to **User Guide > Compute > Instance > Preparing for API Use**.
* **auth_url**
    * Specify the NHN Cloud identity service address.
    * Click the **API Endpoint Settings** button in the NHN Cloud Console's **Compute > Instance > Management** menu to check the identity service URL.
* **region**
