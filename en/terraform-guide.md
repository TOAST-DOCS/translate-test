...
<a id="terraform"></a>
## Terraform
Terraform is an open-source tool that makes it easy to build infrastructure, safely make changes, and efficiently manage infrastructure configuration. The main characteristics of Terraform are as follows.
...
* **Resource Graph**
    ...
    * Create a dependency graph to make plans based on this graph and verify the infrastructure state that changes when the plan is applied.
* **Change Automation**
    * Automate the process of building and modifying infrastructure with the same configuration in multiple locations.
...
* **Component names and version numbers may change, so please verify before use.**


<a id="terraform-installation"></a>
## Terraform Installation
Download the file matching your local PC's operating system from the [Terraform download page](https://www.terraform.io/downloads.html). Extract the file, place it in your desired path, and add that path to your environment settings to complete the installation.

Below is an example installation for Linux (Ubuntu/Debian).
...
Write the provider version by referring to the `VERSION` information on the [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).
...
* **tenant_id**
    * Click the **API Endpoint Settings** button in the NHN Cloud console's **Compute > Instance > Management** menu to verify the tenant ID.
* **password**
    * Use the **API password** saved in the **API Endpoint Settings** dialog box.
    * Refer to **User Guide > Compute > Instance > API Usage Preparation** for how to set the API password.
* **auth_url**
    * Specify the NHN Cloud identity service address.
    * Click the **API Endpoint Settings** button in the NHN Cloud console's **Compute > Instance > Management** menu to verify the identity service URL.
* **region**
