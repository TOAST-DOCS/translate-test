...
Terraform executes by starting with a Terraform binary file and calling the desired target in a local environment or remote environment such as a deployment server. While the "desired targets" have different calling methods, they interact by calling APIs provided by the target's supplier, i.e., the provider. A "provider" is what enables Terraform to interact with targets.
...
First, write the infrastructure configuration to a tf file. Verify the build plan based on the written tf file using the `plan` command as follows:
...
Write tf files on the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a build plan.
