...
Terraform runs by calling the desired target from a local environment or a remote environment such as a deployment server, starting with the Terraform binary file. The 'desired target' uses different calling methods, but interacts by calling the API provided by the target's provider. In this case, the 'provider' is what enables Terraform to interact with the target.
...
First, write the infrastructure configuration in a tf file. Verify the build plan based on the written tf file using the `plan` command as follows.
...
Write tf files on the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file, or write separate tf files for each resource. Terraform reads all written tf files at once and creates a build plan.
