...
Terraform runs by calling a desired target from the terraform binary file in a local environment or a remote environment such as a deployment server. In this case, the desired targets have different calling methods, but they interact by calling the API provided by the target's provider. Here, a 'provider' is what enables Terraform to interact with the target.
...
Infrastructure building with Terraform typically has the following lifecycle:
...
First, write the desired infrastructure configuration in a tf file. Verify the build plan according to the written tf file using the `plan` command as follows:

```
$ terraform plan
```

If the build plan is correct, create, modify, or delete resources using the `apply` command as follows:
...
Write the tf file in the path where the provider configuration file is located. You can gather multiple resource settings in one tf file or write separate tf files for each resource. Terraform reads all written tf files at once to establish a build plan.
