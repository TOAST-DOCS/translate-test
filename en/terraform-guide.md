...
Terraform executes by invoking a desired target from a local environment or remote environment such as a deployment server, starting with the terraform binary file. In this case, the desired targets use different invocation methods, but they interact by calling APIs provided by the target's provider. The 'provider' is what enables Terraform to interact with the target.
...
First, write the infrastructure configuration in a tf file. Verify the construction plan based on the written tf file using the `plan` command as shown below.

```
$ terraform plan
```

If the construction plan is correct, use the `apply` command to create, modify, or delete resources.
