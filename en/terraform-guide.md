...
Terraform executes by starting with the Terraform binary file to call desired targets in either a local environment or a remote environment such as a deployment server. The desired targets have different invocation methods, but they interact by calling APIs provided by the target's provider. The 'provider' is what enables Terraform to interact with the target.
...
Infrastructure construction with Terraform typically follows a lifecycle as shown below.
...
First, write the desired infrastructure configuration in a tf file. Verify the construction plan based on the written tf file using the `plan` command as shown below.
...
Create tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file or create separate tf files for each resource. Terraform reads all written tf files at once to establish a construction plan.
