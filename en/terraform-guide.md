...
* **Resource Graph**
    * Preview how minor changes will impact your entire infrastructure.
    * Create a dependency graph to make plans based on this graph and check the infrastructure state changes when applying the plan.
* **Change Automation**
    * Automate the ability to build and change infrastructure with the same configuration in multiple locations.
...
* **The names and numbers of components including versions may change, so please check and use them accordingly.**
...
* **tenant_id**
    * Click the **API Endpoint Settings** button in the **Compute > Instance > Management** menu in the NHN Cloud console to check the tenant ID.
* **password**
    * Use the **API password** saved in the **API Endpoint Settings** dialog.
    * For how to set the API password, refer to **User Guide > Compute > Instance > Preparing for API Usage**.
* **auth_url**
