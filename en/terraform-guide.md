...
First, write the infrastructure configuration to a tf file. Review the construction plan based on the written tf file using the `plan` command as follows:

```
$ terraform plan
```

If the construction plan looks good, use the `apply` command to create, modify, or delete resources.
...
Write tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in one tf file or write them in separate tf files by resource. Terraform reads all written tf files at once and creates a construction plan.
