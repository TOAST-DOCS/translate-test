...
* **Execution Plan**
    * By separating the change plan from the change application, you can reduce the mistakes that may occur when applying changes.
* **Resource Graph**
    * You can check in advance how minor changes will affect the entire infrastructure.
    * You can create a dependency graph, make plans based on this graph, and verify the infrastructure state that changes when the plan is applied.
* **Change Automation**
...
The provider version is written by referring to the `VERSION` information in the [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).
...
* **tenant_id**
    * Check the tenant ID by clicking the **API Endpoint Settings** button in the **Compute > Instance > Management** menu on the NHN Cloud console.
* **password**
    * Use the **API Password** saved in the **API Endpoint Settings** dialog.
    * For information on how to set the API password, see **User Guide > Compute > Instance > Preparing for API Use**.
* **auth_url**
    * Specifies the NHN Cloud identity service address.
    * Check the identity service URL by clicking the **API Endpoint Settings** button in the **Compute > Instance > Management** menu on the NHN Cloud console.
* **region**
