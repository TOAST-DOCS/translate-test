...
Terraform starts with the Terraform binary file and runs by calling desired targets in a local environment or remote environments such as deployment servers. The desired targets use different invocation methods, but they interact by calling the APIs provided by the target's provider. A "provider" enables Terraform to interact with the target.
...
First, write the infrastructure configuration in tf files. Verify the build plan according to the written tf files using the `plan` command as follows:
...
Write tf files at the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a build plan.
