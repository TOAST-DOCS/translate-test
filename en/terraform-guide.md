...
## Terraform Provider Offering

NHN Cloud, as an official HashiCorp partner, provides Terraform provider through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform executes by calling desired targets in local environments or remote environments like deployment servers, starting with the terraform binary file. The desired targets use different invocation methods, but they interact by calling APIs provided by the target's provider. The 'provider' is what enables Terraform to interact with the target.
...
First, write the infrastructure configuration to tf files. Review the construction plan according to the written tf files using the `plan` command as follows:
...
Write tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a construction plan.
