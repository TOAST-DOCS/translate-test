...
* **Infrastructure as Code**
    ...
    * You can easily share the defined code and collaborate efficiently.
* **Execution Plan**
    * By separating the change plan from the change application, you can reduce the mistakes that can occur when applying changes.
* **Resource Graph**
...
* **Change Automation**
    ...
    * You can save time and reduce errors in infrastructure setup.
...
## Install Terraform
Download the file that matches your local PC's operating system from the [Terraform download page](https://www.terraform.io/downloads.html). Decompress the file, place it in your desired path, and add that path to your environment settings to complete the installation.
...
For the provider version, refer to the `VERSION` information from the [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).
...
* **tenant_id**
    * Click the **API Endpoint Settings** button in the NHN Cloud console at **Compute > Instance > Management** to verify the tenant ID.
* **password**
    * Use the **API password** saved in the **API Endpoint Settings** dialog.
    * For how to set the API password, refer to **User Guide > Compute > Instance > Preparing to Use the API**.
* **auth_url**
    * Specify the NHN Cloud identity service address.
    * Click the **API Endpoint Settings** button in the NHN Cloud console at **Compute > Instance > Management** to verify the identity service URL.
* **region**
