...
Terraform executes by calling the desired target from a local environment or a remote environment such as a deployment server, starting with the terraform binary file. At this time, the desired target has a different way of calling, but it interacts by calling the API provided by the target provider, which is the supplier. Here, a 'provider' enables Terraform to interact with the target.
...
First, write the desired infrastructure configuration in tf files. You can verify the build plan for the written tf files using the `plan` command as follows.

```
$ terraform plan
```

If the build plan is correct, use the `apply` command to create, modify, and delete resources.
...
Write tf files in the path where the provider configuration file is located. You can either consolidate multiple resource configurations into a single tf file or create separate tf files for each resource. Terraform reads all written tf files at once and establishes the build plan.
