...
* **Resource Graph**
    ...
    * You can create a dependency graph, create a plan based on this graph, and check the infrastructure state changes when the plan is applied.
* **Change Automation**
    * You can automate the construction and changes of infrastructure with the same configuration in multiple locations.
...
* **Component names and numbers including version may change, so please verify before using.**


<a id="terraform-installation"></a>
## Install Terraform
Download the file that matches the operating system of your local PC from the [Terraform download page](https://www.terraform.io/downloads.html). Unzip the file, place it in the desired path, and add that path to your environment settings to complete the installation.
...
* **tenant_id**
    * Click the **Set API Endpoint** button in the **Compute > Instance > Manage** menu in the NHN Cloud console to check the tenant ID.
* **password**
    * Use the **API password** saved in the **API Endpoint Setting** dialog box.
    * For how to set the API password, refer to **User Guide > Compute > Instance > Prepare to Use API**.
* **auth_url**
    * Specify the NHN Cloud identity service address.
    * Click the **Set API Endpoint** button in the **Compute > Instance > Manage** menu in the NHN Cloud console to check the identity service URL.
* **region**
