...
Terraform executes starting with the terraform binary file by calling the desired target in a local environment or remote environment such as a deployment server. In this process, the 'desired target' has different invocation methods, but interacts by calling the API provided by the target's provider. Here, the 'provider' is what enables Terraform to interact with the target.
...
Building infrastructure with Terraform typically follows the following lifecycle:

1. Writing tf files
2. Checking build plan
3. Creating resources
4. Modifying resources
5. Deleting resources

First, write the infrastructure configuration to be built in tf files. The build plan based on the written tf files can be checked using the `plan` command as follows:

```
$ terraform plan
```

If the build plan is satisfactory, create, modify, or delete resources using the `apply` command:
...
Write tf files in the path where the provider configuration file is located. Multiple resource configurations can be collected in a single tf file, or separate tf files can be created for each resource. Terraform reads all written tf files at once and establishes a build plan.
