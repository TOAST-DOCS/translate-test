...
Infrastructure building with Terraform typically follows the lifecycle described below.
...
First, write the desired infrastructure configuration in a tf file. Verify the build plan from the written tf file using the `plan` command as follows.

```
$ terraform plan
```

If the build plan looks good, use the `apply` command to create, modify, or delete resources.
...
Write tf files in the path where the provider configuration file is located. You can either collect multiple resource configurations in a single tf file or write separate tf files for each resource. Terraform reads all written tf files at once and creates a build plan.
