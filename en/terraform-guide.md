...
<a id="terraform"></a>
## Terraform
Terraform is an open source tool that allows you to easily build infrastructure, safely change it, and efficiently manage the configuration of your infrastructure. The main features of Terraform are as follows:
...
* **Resource Graph**
    ...
    * You can create a dependency graph, make plans based on this graph, and check the infrastructure state that changes when this plan is applied.
* **Change Automation**
...
## Install Terraform
Download the file suitable for your local PC's operating system from the [Terraform download page](https://www.terraform.io/downloads.html). Extract the file, place it in the desired path, and add that path to your environment settings to complete the installation.
...
For the provider version, refer to the `VERSION` information in [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).
...
* **tenant_id**
    * Check the tenant ID by clicking the **API Endpoint Settings** button in **Compute > Instance > Management** menu of the NHN Cloud console.
* **password**
    * Use the **API password** saved in the **API Endpoint Settings** dialog box.
    * For how to set up the API password, refer to **User Guide > Compute > Instance > API Preparation**.
* **auth_url**
    * Specify the NHN Cloud identity service address.
    * Check the identity service URL by clicking the **API Endpoint Settings** button in **Compute > Instance > Management** menu of the NHN Cloud console.
* **region**
    * Enter the region information where you will manage NHN Cloud resources.
