...
Terraform executes by invoking a desired target in a local environment or remote environment such as a deployment server, starting with the terraform binary file. In this case, the desired target uses different invocation methods, but it interacts by calling the API provided by the target's provider. A 'provider' is what enables Terraform to interact with the target.
...
Building infrastructure using Terraform typically follows a lifecycle as described below:
...
First, write the desired infrastructure configuration in a tf file. Review the build plan based on the written tf file using the `plan` command as follows:

```
$ terraform plan
```

If the build plan is acceptable, use the `apply` command to create, modify, and delete resources.
...
Write tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file, or write them as separate tf files for each resource. Terraform reads all written tf files at once and establishes a build plan.
