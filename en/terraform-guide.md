...
## Terraform Provider
NHN Cloud provides Terraform providers through the [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest) as an official partner of HashiCorp.

Terraform executes by invoking the desired target in a local environment or remote environment such as a deployment server, starting with the terraform binary file. The 'desired target' uses different invocation methods, but interacts by calling the API provided by the provider of the target. The 'provider' is what enables Terraform to interact with the target.
...
First, write the infrastructure configuration in tf files. You can verify the construction plan according to the written tf files using the `plan` command as follows:

```
$ terraform plan
```

If the construction plan is valid, use the `apply` command to create, modify, or delete resources.
...
### Write tf Files
Write tf files in the path where the provider configuration file is located. You can either collect multiple resource configurations in one tf file or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a construction plan.
