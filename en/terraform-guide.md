...
Terraform executes by calling the desired target starting from the terraform binary file in the local environment or in a remote environment such as a deployment server. The desired targets use different invocation methods, but interact by calling APIs provided by the target's provider. A provider enables Terraform to interact with the target.
...
First, write the desired infrastructure configuration in tf files. Review the build plan based on the written tf files using the `plan` command as shown below:
...
Write tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in a single tf file or write separate tf files for each resource. Terraform reads all written tf files at once to establish the build plan.
