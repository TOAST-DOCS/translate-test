...
As an official partner of HashiCorp, NHN Cloud provides the Terraform provider through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform starts with the terraform binary file and runs by calling the desired target in the local environment or in a remote environment such as a deployment server. The 'desired target' has different invocation methods, but it interacts by calling the API provided by the target's provider. The 'provider' is what allows Terraform to interact with the target.
...
Infrastructure building using Terraform typically has the following lifecycle:
...
First, write the configuration of the infrastructure to build in a tf file. Verify the build plan according to the written tf file with the `plan` command as shown below.

```
$ terraform plan
```

If the build plan is without issues, use the `apply` command to create, modify, or delete resources.
...
Write the tf file in the path where the provider configuration file is located. You can either gather multiple resource configurations in one tf file or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a build plan.

The following is an example of a tf file that defines the resource to create an instance in the `instance.tf` file.
