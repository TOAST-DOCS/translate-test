...
Infrastructure construction using Terraform usually has the following lifecycle:
...
First, write the infrastructure configuration to be built in tf files. The construction plan based on the written tf files can be checked with the `plan` command as follows:

```
$ terraform plan
```

If there are no problems with the construction plan, use the `apply` command to create, modify, or delete resources.
