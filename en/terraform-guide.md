...
<a id="terraform"></a>
## Terraform
Terraform is an open source tool that allows you to easily build infrastructure, safely make changes, and efficiently manage infrastructure configuration. The main features of Terraform are as follows:
...
The provider version should be written by referencing the `VERSION` information from [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).
...
* **password**
    ...
    * For how to set up the API password, refer to **User Guide > Compute > Instance > Preparing to Use API**.
* **auth_url**
...
First, write the infrastructure configuration in tf files. You can check the deployment plan based on the written tf files using the `plan` command as follows:

```
$ terraform plan
```

If the deployment plan looks good, use the `apply` command to create, modify, or delete resources.
...
Write tf files in the path where the provider configuration file is located. You can put multiple resource configurations in a single tf file, or create separate tf files for each resource. Terraform reads all written tf files at once to create a deployment plan.
