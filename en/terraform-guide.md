...
This document describes how to use Terraform with NHN Cloud.

<a id="terraform"></a>
## Terraform
Terraform is an open-source tool that makes it easy to build infrastructure, safely change it, and efficiently manage infrastructure configuration. The key characteristics of Terraform are as follows.
...
* **Execution Plan**
    * Separate the change plan from change application to reduce mistakes that can occur when applying changes.
* **Resource Graph**
    * Preview how minor changes will impact the entire infrastructure.
    * Create a dependency graph to plan based on this graph, and verify the infrastructure state that changes when the plan is applied.
* **Change Automation**
    * Automate the process of building and changing infrastructure with the same configuration in multiple locations.
    * Save time building infrastructure and reduce mistakes.
...
* **Component names and version numbers including versions may change, so please verify before use.**
...
The provider file name can be set arbitrarily, and in this example we use `provider.tf`.

The provider version should be written by referring to the `VERSION` information from [NHN Cloud Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).
...
* **tenant_id**
    * In the NHN Cloud Console, go to **Compute > Instance > Management** menu and click the **API Endpoint Settings** button to confirm your tenant ID.
* **password**
    * Use the **API Password** saved in the **API Endpoint Settings** dialog box.
    * For how to set the API password, refer to **User Guide > Compute > Instance > Preparing to Use API**.
* **auth_url**
    * Specify the NHN Cloud identity service address.
    * In the NHN Cloud Console, go to **Compute > Instance > Management** menu and click the **API Endpoint Settings** button to confirm the identity service URL.
* **region**
