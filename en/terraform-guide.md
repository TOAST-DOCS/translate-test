...
Terraform runs by invoking a desired target in a local environment or remote environment such as a deployment server, starting with the Terraform binary file. The desired target may have different invocation methods, but it interacts by calling the API provided by the target's provider. A 'provider' is what enables Terraform to interact with the target.
...
First, write the infrastructure configuration to be built in a tf file. You can verify the construction plan according to the written tf file using the `plan` command as follows:

```
$ terraform plan
```

If the construction plan is satisfactory, create, modify, or delete resources using the `apply` command:
...
### Write tf Files
Write tf files on the path where the provider configuration file is located. You can collect multiple resource configurations in a single tf file or write them as separate tf files for each resource. Terraform reads all written tf files at once and establishes a construction plan.

The following is an example of a tf file that defines resources for creating instances in an `instance.tf` file:
