...
NHN Cloud is an official partner of HashiCorp and provides the Terraform provider through [Terraform Registry](https://registry.terraform.io/providers/nhn-cloud/nhncloud/latest).

Terraform runs by invoking desired targets from a local environment or remote environments such as deployment servers, starting with the terraform binary file. The targets have different invocation methods, but they interact by calling APIs provided by the target provider. A provider is what enables Terraform to interact with the target.
...
First, write the infrastructure configuration in a tf file. You can check the construction plan for the written tf file using the `plan` command as follows:

```
$ terraform plan
```

If the construction plan is correct, use the `apply` command to create, modify, and delete resources.
...
Write tf files in the path where the provider configuration file is located. You can gather multiple resource configurations in one tf file or write separate tf files for each resource. Terraform reads all written tf files at once and establishes a construction plan.
